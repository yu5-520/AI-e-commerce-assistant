...
from src.services.agent_llm_enrichment_service import (
    enrich_module_agent_result,
    enrich_task_generation_result,
    enrich_task_playbook_result,
)
...
def current_agent_plan() -> Dict[str, Any]:
    ...
    plan["mode"] = "module_task_creative_feedback_action_plan_llm_rag_enrichment"
    plan["principle"] = "Agent 不放在最高控制位；模块发现问题，problemType 决定处理包，RAG 召回经验，LLM 只做表达增强和草案细化。"
    ...
    additions = [
        ...
        {"id": "task-generation", "name": "自动解析生成任务 Agent", "module": "task", "output": "规则命中、RAG 引用、置信度、问题类型处理包、LLM 任务说明增强"},
        {"id": "task-playbook", "name": "任务解析运营方式 Agent", "module": "task", "output": "稳健型 / 增长型 / 利润型打法、RAG 案例、LLM 复核说明"},
        {"id": "module-llm-enrichment", "name": "模块方案增强 Agent", "module": "all", "output": "模块 ActionPlan + RAG 召回 + LLM 执行说明 / 复核重点"},
        {"id": "creative-vertical", "name": "标题主图垂直类目 Agent", "module": "product", "output": "标题测试包、主图方向、卖点排序、RAG 经验、LLM 表达增强、上架测试指标"},
        {"id": "feedback-flywheel", "name": "回流任务 Agent", "module": "feedback", "output": "周期摘要、学习候选、经验卡草案、LLM 经验提炼、反馈指标"},
    ]
    ...
    plan["v453LlmRagEnrichment"] = {
        "service": "src/services/agent_llm_enrichment_service.py",
        "rule": "普通模块、任务生成、任务解析和回流草案统一经过 RAG 召回与 LLM 表达增强；problemType 和 ActionPlan 合约仍保持确定性。",
        "outputs": ["retrievedCases", "ragReferences", "llmEnrichment", "llmSummary", "llmOperatorBrief", "llmManagerReviewBrief", "llmRiskCheck", "llmFallbackUsed"],
    }
    plan["v450LlmGateway"] = {
...
def module_agent_task(
    ...
    result["agent"] = enrich_module_agent_result(result.get("agent") or {})
    return result
