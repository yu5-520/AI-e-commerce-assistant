...
def feedback_flywheel(request: Request) -> Dict[str, Any]:
    return feedback_flywheel_summary(user_id=request_user_id(request))
...
def feedback_cycle(request: Request, target: str = "日报", limit: int = Query(default=8, ge=1, le=30)) -> Dict[str, Any]:
    return cycle_feedback_agent(target=target, user_id=request_user_id(request), limit=limit)
