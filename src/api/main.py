"""V16.14 FastAPI entrypoint.

MVP runtime only. Legacy routes, worker scaffold routes, deleted source-core
modules, old mock workflow dependencies, syntax leftovers, and old audit context
imports are removed from the active app; Git history remains the archive.
"""
...
STATION_MAINLINE = {
    "version": API_VERSION,
    "legacyStartupHooks": [],
    "mode": "v1614_audit_context_cleanup",
    "mainline": [
    ...
    "rule": "V16.14：审计路由删除旧src.core.context依赖。当前工作树只服务MVP；FastAPI完整导入继续作为清理守门。",
}
...
def index() -> Any:
    ...
    return {"message": "AI ERP Operating Advisor API is running.", "version": API_VERSION, "v1614": "audit_context_cleanup", "stationMainline": STATION_MAINLINE}
