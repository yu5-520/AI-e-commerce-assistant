...
def role_view_for_user(user: Dict[str, Any]) -> Dict[str, Any]:
    pages = {"owner": {"headline": "老板统筹台", "summary": "先看店群经营盘面，再看员工实时状态、供投财务、组织效率和复盘审计。", "sections": ["店群总览", "人员总览", "供投财务", "组织效率", "复盘审计"]}, "manager": {"headline": "店群总管工作台", "summary": "看负责店群的一线经营模块，拆分任务、派发运营并复核结果。", "sections": ["店群经营", "商品", "竞品", "上新", "流量", "待办", "复核"]}, "operator": {"headline": "运营执行台", "summary": "只看自己的任务、任务报告、提交入口和退回原因。", "sections": ["我的任务", "任务报告", "处理记录"]}, "finance": {"headline": "数据 / 财务台", "summary": "看供货成本、投流消耗、利润、ROI、退款成本和库存资金。", "sections": ["店群总览", "供投财务", "报表异常", "日志"]}, "observer": {"headline": "只读观察台", "summary": "只看摘要、进度和归档日志。", "sections": ["总览", "店群总览", "复盘审计", "日志摘要"]}}
...
def account_summary(user_id: str | None = None) -> Dict[str, Any]:
    ...
    return {"currentUser": user, "currentRoleView": role_view_for_user(user), "roles": list_roles(), "permissions": list_permissions(), "users": list_users(), "storeGroups": list_store_groups(), "stores": list_stores(), "roleChangeLogs": list_role_change_logs(), "taskFlow": ["店群盘面汇总", "人员状态确认", "供投财务确认", "组织权限确认", "复盘审计生成下周期任务"]}
