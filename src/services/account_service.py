...
OPERATION_MODULES = ["dashboard", "operating-unit", "data-check", "business-products", "business-competitors", "business-listing", "business-traffic", "business-actions", "business-report", "accounts"]

ROLES: List[Dict[str, Any]] = [
    {"id": "owner", "name": "老板账号", "level": 1, "scope": "全部店群", "insightDepth": "owner_strategy", "insightName": "老板统筹视角", "description": "看店群盘面、人员状态、供投财务、组织效率和审计。", "permissions": ["view_all_stores", "view_finance", "view_org_risk", "view_command", "dispatch_tasks", "review_tasks", "manage_roles", "view_only"], "visibleModules": EXECUTIVE_MODULES, "allowedActions": ["查看店群总览", "查看人员总览", "查看供投财务", "查看组织效率", "管理角色权限"], "hiddenFields": ["一线商品操作台", "一线上新操作台", "一线流量操作台", "一线竞品操作台"], "managementInsights": ["平台盘面", "店铺经营", "人员状态", "供投财务", "组织效率", "复核审计"]},
    {"id": "manager", "name": "店群总管账号", "level": 2, "scope": "负责店群", "insightDepth": "team_management", "insightName": "店群管理视角", "description": "拆分任务、派发运营并复核结果。", "permissions": ["view_managed_stores", "view_finance", "assign_tasks", "review_tasks", "manage_roles", "view_only"], "visibleModules": OPERATION_MODULES + ["role-console"], "allowedActions": ["拆分任务", "派发运营", "调整优先级", "复核通过", "退回补充"], "hiddenFields": ["全公司预算决策", "跨店群组织判断"], "managementInsights": ["店群任务积压", "运营处理质量", "重复预警", "复核效率", "流程卡点"]},
    {"id": "operator", "name": "运营账号", "level": 3, "scope": "自己的任务", "insightDepth": "execution_checklist", "insightName": "执行处理视角", "description": "只看自己的任务、报告、检查清单和提交入口。", "permissions": ["view_own_tasks", "handle_tasks", "submit_tasks", "view_only"], "visibleModules": ["dashboard", "business-actions", "business-report", "accounts"], "allowedActions": ["查看任务报告", "提交处理结果", "补充说明", "重新提交"], "hiddenFields": ["全部店群", "其他运营任务", "财务利润", "组织判断"], "managementInsights": ["任务原因", "检查字段", "提交证据", "退回补充"]},
    {"id": "finance", "name": "数据 / 财务账号", "level": 3, "scope": "报表与财务数据", "insightDepth": "finance_risk", "insightName": "财务经营视角", "description": "查看供货成本、投流消耗、ROI、退款成本、库存资金和财务汇总。", "permissions": ["view_finance", "view_only"], "visibleModules": ["dashboard", "store-overview", "profit-budget", "data-check", "business-report", "accounts"], "allowedActions": ["查看供投财务", "标记数据异常", "补充财务说明"], "hiddenFields": ["任务派发按钮", "运营复核按钮", "角色管理"], "managementInsights": ["供货成本", "广告消耗", "ROI 可信度", "库存资金"]},
    {"id": "observer", "name": "只读观察账号", "level": 4, "scope": "被授权范围", "insightDepth": "summary_only", "insightName": "只读摘要视角", "description": "只能查看摘要、进度和日志结果。", "permissions": ["view_only"], "visibleModules": ["dashboard", "store-overview", "review-audit", "business-report", "accounts"], "allowedActions": ["查看摘要", "查看日志"], "hiddenFields": ["财务细节", "任务责任链", "全部操作按钮"], "managementInsights": ["平台摘要", "店铺状态", "处理进度", "归档结果"]},
]
...
def get_user(user_id: str | None) -> Dict[str, Any] | None:
    user = next((item for item in USERS if item["id"] == user_id), None) if user_id else None
...
def user_id_from_headers(headers: Mapping[str, str] | None = None, fallback: str | None = None) -> str:
    headers = headers or {}; return resolve_user_id(headers.get("x-mock-user-id") or headers.get("X-Mock-User-Id") or fallback)
...
def user_display(user_id: str | None, fallback: str = "未派发") -> str:
    user = get_user(user_id); return fallback if not user else f"{user['name']} · {user['roleName']}"
...
def role_view_for_user(user: Dict[str, Any]) -> Dict[str, Any]:
    pages = {"owner": {"headline": "老板统筹台", "summary": "先看店群经营盘面，再看员工实时状态、供投财务、组织效率和审计。", "sections": ["店群总览", "人员总览", "供投财务", "组织效率", "复核审计"]}, "manager": {"headline": "店群总管工作台", "summary": "看负责店群的一线经营模块，拆分任务、派发运营并复核结果。", "sections": ["店群经营", "商品", "竞品", "上新", "流量", "待办", "复核"]}, "operator": {"headline": "运营执行台", "summary": "只看自己的任务、任务报告、提交入口和退回原因。", "sections": ["我的任务", "任务报告", "处理记录"]}, "finance": {"headline": "数据 / 财务台", "summary": "看供货成本、投流消耗、利润、ROI、退款成本和库存资金。", "sections": ["店群总览", "供投财务", "报表异常", "日志"]}, "observer": {"headline": "只读观察台", "summary": "只看摘要、进度和归档日志。", "sections": ["总览", "店群总览", "复核审计", "日志摘要"]}}
...
def account_summary(user_id: str | None = None) -> Dict[str, Any]:
    ...
    return {"currentUser": user, "currentRoleView": role_view_for_user(user), "roles": list_roles(), "permissions": list_permissions(), "users": list_users(), "storeGroups": list_store_groups(), "stores": list_stores(), "roleChangeLogs": list_role_change_logs(), "taskFlow": ["店群盘面汇总", "人员状态确认", "供投财务确认", "异常字段进入任务池", "运营提交后复核归档"]}
