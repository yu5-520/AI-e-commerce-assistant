...
def _run(coro: Any) -> Dict[str, Any]:
    ...
    return {"version": TASK_REPOSITORY_MIRROR_VERSION, "mirrored": False, "status": "skipped", "reason": "event_loop_running; use async mirror path later"}
...
async def _soft_delete_async(ctx: UserContext, reason: str) -> Dict[str, Any]:
    async with get_session_factory()() as session:
        repo = ProductionTaskRepository(session, ctx)
        deleted = await repo.soft_delete_visible(reason=reason)
        await session.commit()
    write_audit_log(ctx, trace_id="TASKRESET_MIRROR", event_type="task.production_soft_deleted", resource_type="decision_task", resource_id="visible_tasks", action="reset_tasks", status="mirrored", payload={"softDeletedTasks": deleted, "mode": repository_mode()})
...
def mirror_task_to_production(ctx: UserContext, task: Dict[str, Any] | None, *, action: str) -> Dict[str, Any]:
    ...
    try:
        return _run(_mirror_task_async(ctx, task, action))
...
def mirror_task_reset_to_production(ctx: UserContext, *, reason: str) -> Dict[str, Any]:
    mode = repository_mode()
    if mode == "sqlite":
        return _skipped("reset_tasks")
    try:
        return _run(_soft_delete_async(ctx, reason))
    except Exception as exc:  # noqa: BLE001
...
def task_repository_mirror_summary() -> Dict[str, Any]:
    return {
        "version": TASK_REPOSITORY_MIRROR_VERSION,
        "mode": repository_mode(),
        "enabled": repository_mode() in {"hybrid", "postgres"},
        "sqliteFirst": True,
        "productionRepository": production_repository_summary(),
