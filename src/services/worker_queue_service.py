"""Worker / Redis queue scaffold.

The current MVP still executes imports synchronously. This service adds a durable
queue contract that can later be backed by Redis / ARQ workers without changing
business API payloads again.
"""
...
def enqueue_worker_job(ctx: UserContext, payload: Dict[str, Any]) -> Dict[str, Any]:
    """Create a queued worker job with idempotency protection."""
    ...
    with connect() as conn:
        if idempotency_key:
            existing = conn.execute(
                "SELECT * FROM worker_jobs WHERE tenant_id = ? AND idempotency_key = ? AND deleted_at IS NULL",
                (ctx.tenant_id, idempotency_key),
            ).fetchone()
            if existing:
                return {"version": WORKER_QUEUE_VERSION, "created": False, "job": _row_to_worker_job(existing)}
        ...
        conn.execute(
            ...
            INSERT INTO worker_jobs (
                ...
                payload, created_by, created_at, updated_at, next_run_at
            ...
            (
                ...
                int(payload.get("maxAttempts") or payload.get("max_attempts") or 3),
                idempotency_key,
                payload.get("correlationId") or payload.get("correlation_id"),
                dumps(job_payload),
                ctx.user_id,
                payload.get("nextRunAt") or payload.get("next_run_at"),
            ),
        )
        row = conn.execute("SELECT * FROM worker_jobs WHERE worker_job_id = ?", (job_id,)).fetchone()
        conn.commit()
    return {"version": WORKER_QUEUE_VERSION, "created": True, "job": _row_to_worker_job(row)}


def claim_next_worker_job(ctx: UserContext, *, queue_name: str = "default", worker_id: str = "worker-demo") -> Dict[str, Any] | None:
    """Claim one queued job for a worker process."""
    ...
    with connect() as conn:
        ...
        conn.commit()
    return {"version": WORKER_QUEUE_VERSION, "job": _row_to_worker_job(claimed)}
    ...
    with connect() as conn:
        conn.execute(
            ...
            SET status = 'completed', result = ?, error_message = NULL,
                finished_at = {_now_expr()}, updated_at = {_now_expr()}
            WHERE worker_job_id = ? AND tenant_id = ? AND deleted_at IS NULL
            """,
            (dumps(result or {}), worker_job_id, ctx.tenant_id),
        )
        row = conn.execute("SELECT * FROM worker_jobs WHERE worker_job_id = ? AND tenant_id = ?", (worker_job_id, ctx.tenant_id)).fetchone()
        conn.commit()
    return {"version": WORKER_QUEUE_VERSION, "job": _row_to_worker_job(row)} if row else None
...
def fail_worker_job(ctx: UserContext, worker_job_id: str, *, error_message: str, retry: bool = True) -> Dict[str, Any] | None:
    ...
    with connect() as conn:
        ...
        conn.execute(
            f"""
            UPDATE worker_jobs
            SET status = ?, error_message = ?, next_run_at = CASE WHEN ? THEN datetime('now', '+1 minute') ELSE next_run_at END,
                finished_at = {finished_expr}, updated_at = {_now_expr()}
            WHERE worker_job_id = ? AND tenant_id = ?
            """,
            (next_status, error_message, 1 if can_retry else 0, worker_job_id, ctx.tenant_id),
        )
        updated = conn.execute("SELECT * FROM worker_jobs WHERE worker_job_id = ?", (worker_job_id,)).fetchone()
        conn.commit()
    return {"version": WORKER_QUEUE_VERSION, "job": _row_to_worker_job(updated)}
...
def retry_worker_job(ctx: UserContext, worker_job_id: str) -> Dict[str, Any] | None:
    ...
    with connect() as conn:
        conn.execute(
            f"""
            UPDATE worker_jobs
            SET status = 'queued', error_message = NULL, next_run_at = {_now_expr()}, updated_at = {_now_expr()}
            WHERE worker_job_id = ? AND tenant_id = ? AND deleted_at IS NULL AND status IN ('failed', 'retry_scheduled')
            """,
            (worker_job_id, ctx.tenant_id),
        )
        row = conn.execute("SELECT * FROM worker_jobs WHERE worker_job_id = ? AND tenant_id = ?", (worker_job_id, ctx.tenant_id)).fetchone()
        conn.commit()
    return {"version": WORKER_QUEUE_VERSION, "job": _row_to_worker_job(row)} if row else None
...
def list_worker_jobs(ctx: UserContext, *, queue_name: str | None = None, status: str | None = None, limit: int = 100) -> Dict[str, Any]:
    ...
    with connect() as conn:
        rows = conn.execute(
            f"SELECT * FROM worker_jobs WHERE {' AND '.join(where)} ORDER BY created_at DESC LIMIT ?",
            params,
        ).fetchall()
...
def worker_queue_summary(ctx: UserContext) -> Dict[str, Any]:
    ...
    return {
        ...
        "mode": "sqlite_worker_queue_scaffold",
        "tenantId": ctx.tenant_id,
        "counts": counts,
        "activeTotal": sum(counts.get(status, 0) for status in ACTIVE_STATUSES),
        "terminalTotal": sum(counts.get(status, 0) for status in TERMINAL_STATUSES),
        "supportedJobTypes": sorted(SUPPORTED_JOB_TYPES),
        "nextStep": "Replace SQLite queue operations with Redis / ARQ worker execution while keeping this API contract.",
    }
