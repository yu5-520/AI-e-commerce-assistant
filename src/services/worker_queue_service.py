"""Worker / Redis queue scaffold with trace audit and hybrid mirror."""
...
from src.services.import_worker_repository_mirror_service import mirror_worker_job_to_production
from src.services.trace_audit_service import resolve_trace_id, write_audit_log

WORKER_QUEUE_VERSION = "5.3.3"
    ...
    with connect() as conn:
        conn.execute("""
            CREATE TABLE IF NOT EXISTS worker_jobs (
                worker_job_id TEXT PRIMARY KEY, tenant_id TEXT DEFAULT 'tenant_demo', org_id TEXT DEFAULT 'org_demo',
                queue_name TEXT NOT NULL, job_type TEXT NOT NULL, status TEXT NOT NULL, priority INTEGER DEFAULT 50,
                attempt_count INTEGER DEFAULT 0, max_attempts INTEGER DEFAULT 3, idempotency_key TEXT,
                correlation_id TEXT, payload TEXT, result TEXT, error_message TEXT, created_by TEXT, claimed_by TEXT,
                created_at TEXT NOT NULL, updated_at TEXT NOT NULL, claimed_at TEXT, finished_at TEXT,
                next_run_at TEXT, deleted_at TEXT
            )
        """)
...
def _row_to_worker_job(row: Any) -> Dict[str, Any]:
    keys = row.keys()
    return {"workerJobId": row["worker_job_id"], "traceId": row["trace_id"] if "trace_id" in keys else None, "tenantId": row["tenant_id"], "orgId": row["org_id"], "queueName": row["queue_name"], "jobType": row["job_type"], "status": row["status"], "priority": row["priority"], "attemptCount": row["attempt_count"], "maxAttempts": row["max_attempts"], "idempotencyKey": row["idempotency_key"], "correlationId": row["correlation_id"], "payload": loads(row["payload"]), "result": loads(row["result"]), "errorMessage": row["error_message"], "createdBy": row["created_by"], "claimedBy": row["claimed_by"], "createdAt": row["created_at"], "updatedAt": row["updated_at"], "claimedAt": row["claimed_at"], "finishedAt": row["finished_at"], "nextRunAt": row["next_run_at"]}


def _mirror(ctx: UserContext, job: Dict[str, Any] | None, action: str) -> Dict[str, Any]:
    return mirror_worker_job_to_production(ctx, job, action=action)
...
def enqueue_worker_job(ctx: UserContext, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with connect() as conn:
        if idempotency_key:
            ...
            if existing:
                ...
                return {"version": WORKER_QUEUE_VERSION, "created": False, "job": job, "productionMirror": _mirror(ctx, job, "worker_job.idempotent_reused")}
    ...
    return {"version": WORKER_QUEUE_VERSION, "created": True, "job": job, "productionMirror": _mirror(ctx, job, "worker_job.queued")}
...
def claim_next_worker_job(ctx: UserContext, *, queue_name: str = "default", worker_id: str = "worker-demo") -> Dict[str, Any] | None:
    ...
    with connect() as conn:
        row = conn.execute(f"""
            ...
            LIMIT 1
        """, (ctx.tenant_id, queue_name)).fetchone()
        if not row:
            return None
        conn.execute(f"""
            UPDATE worker_jobs
    ...
    return {"version": WORKER_QUEUE_VERSION, "job": job, "productionMirror": _mirror(ctx, job, "worker_job.claimed")}
...
def complete_worker_job(ctx: UserContext, worker_job_id: str, result: Dict[str, Any] | None = None) -> Dict[str, Any] | None:
    ...
    return {"version": WORKER_QUEUE_VERSION, "job": job, "productionMirror": _mirror(ctx, job, "worker_job.completed")}
...
def fail_worker_job(ctx: UserContext, worker_job_id: str, *, error_message: str, retry: bool = True) -> Dict[str, Any] | None:
    ...
    return {"version": WORKER_QUEUE_VERSION, "job": job, "productionMirror": _mirror(ctx, job, f"worker_job.{next_status}")}
...
def retry_worker_job(ctx: UserContext, worker_job_id: str) -> Dict[str, Any] | None:
    ...
    return {"version": WORKER_QUEUE_VERSION, "job": job, "productionMirror": _mirror(ctx, job, "worker_job.retry")}
...
def worker_queue_summary(ctx: UserContext) -> Dict[str, Any]:
    ...
    return {"version": WORKER_QUEUE_VERSION, "mode": "sqlite_worker_queue_with_hybrid_mirror", "tenantId": ctx.tenant_id, "counts": counts, "activeTotal": sum(counts.get(status, 0) for status in ACTIVE_STATUSES), "terminalTotal": sum(counts.get(status, 0) for status in TERMINAL_STATUSES), "supportedJobTypes": sorted(SUPPORTED_JOB_TYPES), "nextStep": "WorkerJob writes can mirror to PostgreSQL in hybrid/postgres mode."}
