...
V11.7 separates business-object ingestion from task generation:

- report rows are cleaned and classified first;
- product/store master objects are upserted immediately after import;
- tags and executable tasks are downstream states, not prerequisites for showing
  products or stores in the operating unit page.
"""
...
OPERATING_OBJECT_VERSION = "11.7.0"
...
def _store_name(row: Dict[str, Any]) -> str | None:
...
def _store_key(row: Dict[str, Any]) -> str | None:
...
def _merge_payload(existing: Dict[str, Any], row: Dict[str, Any], *, data_version: str | None, dataset_name: str | None) -> Dict[str, Any]:
...
def upsert_operating_objects_from_import(result: Dict[str, Any], rows: List[Dict[str, Any]] | None = None, *, source: str = "report_import") -> Dict[str, Any]:
    ...
    with connect() as conn:
        for item in _import_result_items(result):
            ...
            for row in _rows_for_result_item(item, rows):
                ...
                store_key = _store_key(row)
                store_id = _store_id(row) or store_key
                store_name = _store_name(row) or (store_id if store_id and not str(store_id).startswith("STORE_") else "未归属店铺")
                ...
                if store_key:
                    ...
                    store_payload = _merge_payload(loads(existing_store["payload"]) if existing_store else {}, row, data_version=data_version, dataset_name=dataset_name)
                    ...
                    conn.execute(
                        ...
                        """,
                        (store_key, store_id, store_name, platform, data_version, dataset_name, store_key, dumps(store_payload), existing_store["first_seen_at"] if existing_store else now, now),
                    )
                ...
                if product_id:
                    ...
                    product_payload = _merge_payload(loads(existing_product["payload"]) if existing_product else {}, row, data_version=data_version, dataset_name=dataset_name)
                    ...
                    conn.execute(
                        ...
                        """,
                        (object_id, product_id, store_id, store_name, platform, title, category, data_version, dataset_name, dumps(product_payload), existing_product["first_seen_at"] if existing_product else now, now),
                    )
                    product_ids.add(object_id)
        for store_key in store_keys:
            count = conn.execute("SELECT COUNT(*) AS count FROM operating_products WHERE store_id = ?", (store_key,)).fetchone()["count"]
    ...
    return {
        ...
        "rule": "商品/店铺主档在报表清洗后立即入库，不依赖标签或任务生成。",
    }
