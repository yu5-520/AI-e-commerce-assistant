"""V16.5 task generation run and split-station data line service."""
...
TASK_GENERATION_RUN_VERSION = "16.5"
...
def record_task_generation_run(*, data_version: str | None, input_bundle_count: int = 0, agent_judgment_count: int = 0, product_judgment_package_count: int = 0, identity_gap_count: int = 0, task_decision_count: int = 0, by_decision: Dict[str, int] | None = None, streamed_task_snapshot_count: int = 0, task_pool_created_count: int = 0, skipped_formal_count: int = 0, zero_task_reasons: List[str] | None = None, agent1_api_call_count: int = 0, rag_retrieval_count: int = 0, api_budget_violation: bool = False, agent_budget_summary: Dict[str, Any] | None = None, total_agent_call_count: int = 0, total_agent_budget: int = 8, source: str = "station_alignment_v165") -> Dict[str, Any]:
    ...
    coverage_rate = round(product_judgment_package_count / input_bundle_count, 4) if input_bundle_count else 0
    coverage_status = "passed" if not input_bundle_count or coverage_rate >= COVERAGE_THRESHOLD else "failed"
    if api_budget_violation:
        status = "attention_agent_budget_violation"
        reason = f"本轮 Agent 调用 {total_agent_call_count}/{total_agent_budget}，预算异常。"
    elif coverage_status == "failed":
        status = "attention_product_judgment_coverage_low"
        reason = f"商品判断覆盖不足：{product_judgment_package_count}/{input_bundle_count}，任务映射暂停。"
    elif task_pool_created_count:
        status = "completed_with_product_tasks"
        reason = f"V16.5拆分站点链路完成，{agent_judgment_count} 条判断整合成 {product_judgment_package_count} 个商品判断包，生成 {task_pool_created_count} 个任务。"
    elif formal_task_count:
        status = "completed_no_pool_entries"
        reason = "任务映射已有正式决策，但未成功进入任务池，需要检查 task_pool_admission_station。"
    else:
        status = "completed_no_formal_task"
        reason = f"V16.5拆分站点链路完成，{agent_judgment_count} 条判断整合成 {product_judgment_package_count} 个商品判断包，本轮无正式任务。"
    ...
    payload = {"version": TASK_GENERATION_RUN_VERSION, "runId": run_id, "dataVersion": data_version, "status": status, "source": source, "inputBundleCount": int(input_bundle_count or 0), "agentJudgmentCount": int(agent_judgment_count or 0), "averageJudgmentsPerBundle": avg, "metricJudgmentMode": "split_station_real_agent", "agent1ApiCallCount": int(agent1_api_call_count or 0), "agent1ApiBudget": int(input_bundle_count or 0), "agent1ApiCallsPerBundle": api_per_bundle, "totalAgentCallCount": int(total_agent_call_count or 0), "totalAgentBudget": int(total_agent_budget or 8), "apiBudgetViolation": bool(api_budget_violation), "agentBudgetLedger": agent_budget_summary, "ragRetrievalCount": int(rag_retrieval_count or 0), "ragRetrievalScope": "data_version_once_or_reused", "productJudgmentPackageCount": int(product_judgment_package_count or 0), "productJudgmentCoverageRate": coverage_rate, "productJudgmentCoverageStatus": coverage_status, "identityGapCount": int(identity_gap_count or 0), "taskDecisionCount": int(task_decision_count or 0), "formalTaskCount": int(formal_task_count or 0), "observeOnlyCount": int(observe_only_count or 0), "dataGapTaskCount": int(data_gap_task_count or 0), "managerReviewCount": int(manager_review_count or 0), "streamedTaskSnapshotCount": int(streamed_task_snapshot_count or 0), "taskPoolCreatedCount": int(task_pool_created_count or 0), "skippedFormalCount": int(skipped_formal_count or 0), "byDecision": by_decision, "reason": reason, "zeroTaskReasons": zero_task_reasons or [], "createdAt": now, "updatedAt": now, "rule": "V16.5: split stations expose coverage, package gate, task mapping, admission and read-model acceptance separately."}
    with connect() as conn:
        frontend_task_count = _count_by_version(conn, "frontend_task_view", data_version)
...
def _latest_product_master_count(conn: Any, data_version: str | None = None) -> int:
    if not _table_exists(conn, "system_product_snapshots_v14"):
        return 0
    if data_version:
        row = conn.execute("SELECT product_count, payload FROM system_product_snapshots_v14 WHERE data_version = ? ORDER BY updated_at DESC LIMIT 1", (data_version,)).fetchone()
    else:
        row = conn.execute("SELECT product_count, payload FROM system_product_snapshots_v14 ORDER BY updated_at DESC LIMIT 1").fetchone()
    if not row:
        return 0
    return int(row["product_count"] or 0)
...
def _headline(*, bundle_count: int, raw_count: int, package_count: int, run_task_count: int, coverage_status: str = "passed", total_calls: int = 0, total_budget: int = 8, residual: bool = False) -> str:
    if residual:
        return "运行态有残留，请重新清空演示数据"
    if bundle_count <= 0:
        return "等待数据接入"
    if raw_count <= 0:
        return "数据已建档，等待商品判断 Agent"
    if package_count <= 0:
        return "商品判断完成，等待判断包合并"
    if coverage_status == "failed":
        return f"商品判断覆盖不足：{package_count}/{bundle_count}，任务映射已暂停"
    if run_task_count <= 0:
...
def read_data_line_status() -> Dict[str, Any]:
    ...
    with connect() as conn:
        ...
        product_master_count = _latest_product_master_count(conn, data_version)
        ...
        pool_total_count = _count_by_version(conn, "task_pool_entries", data_version)
        frontend_task_count = _count_by_version(conn, "frontend_task_view", data_version)
        frontend_product_count = _count_by_version(conn, "frontend_product_view", data_version)
        ...
        agent_budget_ledger = {}
        coverage_rate = round(package_count / bundle_count, 4) if bundle_count else 0
        coverage_status = "passed" if not bundle_count or coverage_rate >= COVERAGE_THRESHOLD else "failed"
        if latest_run:
            ...
            coverage_rate = latest_run.get("productJudgmentCoverageRate") if latest_run.get("productJudgmentCoverageRate") is not None else coverage_rate
            coverage_status = latest_run.get("productJudgmentCoverageStatus") or coverage_status
        upstream_empty = bundle_count == 0 and product_master_count == 0 and frontend_product_count == 0 and pool_total_count == 0 and task_decision_count == 0 and package_count == 0
        ...
        import_status = "passed" if bundle_count or product_master_count or frontend_product_count else "waiting"
        schema_status = "passed" if import_status == "passed" else "waiting"
        fact_status = "passed" if product_master_count or bundle_count else ("current" if import_status == "passed" else "waiting")
        master_status = "passed" if product_master_count else ("current" if fact_status == "passed" else "waiting")
        metric_status = "passed" if product_master_count else master_status
        bundle_status = "passed" if bundle_count else ("current" if product_master_count else "waiting")
        validation_status = "passed" if bundle_count else bundle_status
        ...
        package_status = "error" if residual or coverage_status == "failed" else "passed" if package_count else ("current" if raw_judgment_count else "waiting")
        rag_status = "passed" if package_status == "passed" else "waiting"
        mapping_status = "passed" if task_decision_count else ("waiting" if coverage_status == "failed" else "current" if package_status == "passed" else "waiting")
        if residual:
            task_status = "error"
        elif coverage_status == "failed":
            task_status = "waiting"
            ...
            task_status = "waiting"
        view_status = "passed" if frontend_task_count or frontend_product_count or raw_judgment_count else "waiting"
        acceptance_status = "passed" if run_task_count == frontend_task_count and task_status in {"passed", "empty", "waiting"} else "attention" if run_task_count or frontend_task_count else "waiting"
        line_status = "attention" if residual or api_budget_violation or coverage_status == "failed" or acceptance_status == "attention" else "completed" if package_count or run_task_count else "processing" if bundle_count else "waiting"
        if formal_decision_count > 0 and run_task_count <= 0:
            line_status = "attention"
        headline = _headline(bundle_count=bundle_count, raw_count=raw_judgment_count, package_count=package_count, run_task_count=run_task_count, coverage_status=coverage_status, total_calls=total_agent_call_count, total_budget=total_agent_budget, residual=residual)
        stations = [
            _station("report_receive_station", "接入", import_status, "数据入库"),
            _station("report_schema_station", "结构", schema_status, "表头/日期"),
            _station("report_fact_station", "事实", fact_status, "商品/店铺/流量"),
            _station("product_master_station", "主档", master_status, f"商品 {product_master_count or bundle_count}"),
            _station("product_metric_snapshot_station", "指标", metric_status, "ROI/日期"),
            _station("full_product_bundle_station", "全量包", bundle_status, f"{bundle_count} 个包"),
            _station("bundle_validation_station", "验收", validation_status, "事实层"),
            _station("product_judgment_agent_station", "判断", judgment_status, f"{raw_judgment_count} 条/API {agent1_api_call_count}" if raw_judgment_count else "等待"),
            _station("product_judgment_package_station", "合包", package_status, f"{package_count}/{bundle_count}" if bundle_count else "等待"),
            _station("rag_permission_context_station", "RAG", rag_status, f"检索 {rag_retrieval_count}"),
            _station("task_mapping_agent_station", "映射", mapping_status, f"决策 {task_decision_count}" if task_decision_count else "等待"),
            _station("task_pool_admission_station", "入池", task_status, f"本轮 {run_task_count}" if run_task_count else "无正式任务" if task_status == "empty" else "暂停" if coverage_status == "failed" else "等待"),
            _station("frontend_read_model_station", "读模", view_status, f"任务 {frontend_task_count}"),
            _station("task_pool_acceptance_station", "验收", acceptance_status, "已对齐" if acceptance_status == "passed" else "待验收"),
        ]
        return {"version": TASK_GENERATION_RUN_VERSION, "ready": bool(bundle_count or raw_judgment_count or latest_run), "lineStatus": line_status, "headline": headline, "dataVersion": data_version, "formalTaskCount": int(run_task_count or 0), "taskPoolTotalCount": int(pool_total_count or 0), "formalJudgmentCount": int(formal_decision_count or 0), "observeOnlyCount": int(observe_count or 0), "identityGapCount": int(identity_gap_count or 0), "averageJudgmentsPerBundle": average_judgments_per_bundle, "agent1ApiCallCount": agent1_api_call_count, "totalAgentCallCount": total_agent_call_count, "totalAgentBudget": total_agent_budget, "apiBudgetViolation": api_budget_violation, "ragRetrievalCount": rag_retrieval_count, "agentBudgetLedger": agent_budget_ledger, "agentJudgmentCount": int(raw_judgment_count or 0), "rawJudgmentCount": int(raw_judgment_count or 0), "productJudgmentPackageCount": int(package_count or 0), "productJudgmentCoverageRate": coverage_rate, "productJudgmentCoverageStatus": coverage_status, "taskDecisionCount": int(task_decision_count or 0), "inputBundleCount": int(bundle_count or 0), "productMasterCount": int(product_master_count or 0), "frontendTaskViewCount": int(frontend_task_count or 0), "frontendProductViewCount": int(frontend_product_count or 0), "runtimeResidual": residual, "staleRunIgnored": stale_run_only, "stations": stations, "stationAlignment": "v16_5_split_chain", "latestRun": latest_run, "decisionCounts": decision_counts, "updatedAt": now_iso(), "rule": "V16.5 data-line is generated from the same split station chain as registry/contract/queue/adapter."}
