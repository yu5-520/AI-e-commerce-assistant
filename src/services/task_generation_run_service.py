...
def _decision_counts(conn: Any) -> Dict[str, int]:
...
def read_data_line_status() -> Dict[str, Any]:
    ...
    with connect() as conn:
        ...
        raw_judgment_count = _count(conn, "agent_product_judgments_v15") or _count(conn, "agent_judgments_v14")
        package_count = _count(conn, "product_judgment_packages_v15")
        task_decision_count = _count(conn, "task_generation_decisions_v15")
        pool_count = _count(conn, "task_pool_entries")
        frontend_task_count = _count(conn, "frontend_task_view")
        frontend_product_count = _count(conn, "frontend_product_view")
        decision_counts = _decision_counts(conn)
