"""V14.9.4 task generation run and data metro-line status service.

The data line reports current-run task count and also exposes Agent1 API/RAG
budget counters. Metric judgments can be many, but API calls must stay bounded
by product bundles, never by judgment count.
"""
...
TASK_GENERATION_RUN_VERSION = "14.9.4"
...
def record_task_generation_run(*, data_version: str | None, input_bundle_count: int = 0, agent_judgment_count: int = 0, product_judgment_package_count: int = 0, identity_gap_count: int = 0, task_decision_count: int = 0, by_decision: Dict[str, int] | None = None, streamed_task_snapshot_count: int = 0, task_pool_created_count: int = 0, skipped_formal_count: int = 0, zero_task_reasons: List[str] | None = None, agent1_api_call_count: int = 0, rag_retrieval_count: int = 0, api_budget_violation: bool = False, source: str = "agent_judgment_station") -> Dict[str, Any]:
    ...
    if api_budget_violation:
        status = "attention_api_budget_violation"
        reason = f"Agent1 API调用 {agent1_api_call_count} 次超过商品包 {input_bundle_count} 个，已标记为预算异常。"
    elif task_pool_created_count:
        status = "completed_with_product_tasks"
        reason = f"双 Agent 链路已完成，{agent_judgment_count} 条指标判断整合成 {product_judgment_package_count} 个商品判断包，生成 {task_pool_created_count} 个正式任务。"
        ...
        reason = f"双 Agent 链路已完成，{agent_judgment_count} 条指标判断整合成 {product_judgment_package_count} 个商品判断包，本轮无正式执行任务。"
...
def _headline(*, bundle_count: int, raw_count: int, package_count: int, run_task_count: int, api_calls: int = 0, residual: bool = False) -> str:
    if residual:
    ...
    if run_task_count <= 0:
        return f"{raw_count} 条判断已整合为 {package_count} 个商品判断包，Agent1 API {api_calls} 次，暂无正式任务"
    return f"{raw_count} 条判断已整合为 {package_count} 个商品判断包，Agent1 API {api_calls} 次，本轮生成 {run_task_count} 个正式任务"
...
def read_data_line_status() -> Dict[str, Any]:
    ...
    with connect() as conn:
        ...
        agent1_api_budget = bundle_count
        ...
        if latest_run:
            ...
            agent1_api_budget = int(latest_run.get("agent1ApiBudget") or bundle_count or 0)
            api_budget_violation = bool(latest_run.get("apiBudgetViolation")) or agent1_api_call_count > max(agent1_api_budget, 0)
        ...
        if stale_run_only:
            latest_run = None
            data_version = None
            raw_judgment_count = package_count = task_decision_count = observe_count = formal_decision_count = run_task_count = identity_gap_count = agent1_api_call_count = rag_retrieval_count = 0
            average_judgments_per_bundle = 0
            api_budget_violation = False
        ...
        headline = _headline(bundle_count=bundle_count, raw_count=raw_judgment_count, package_count=package_count, run_task_count=run_task_count, api_calls=agent1_api_call_count, residual=residual)
        ...
        return {"version": TASK_GENERATION_RUN_VERSION, "ready": bool(bundle_count or raw_judgment_count or latest_run), "lineStatus": line_status, "headline": headline, "dataVersion": data_version, "formalTaskCount": int(run_task_count or 0), "taskPoolTotalCount": int(pool_total_count or 0), "formalJudgmentCount": int(formal_decision_count or 0), "observeOnlyCount": int(observe_count or 0), "identityGapCount": int(identity_gap_count or 0), "averageJudgmentsPerBundle": average_judgments_per_bundle, "agent1ApiCallCount": agent1_api_call_count, "agent1ApiBudget": agent1_api_budget, "apiBudgetViolation": api_budget_violation, "ragRetrievalCount": rag_retrieval_count, "dataGapTaskCount": int(latest_run.get("dataGapTaskCount") or 0) if latest_run else 0, "agentJudgmentCount": int(raw_judgment_count or 0), "rawJudgmentCount": int(raw_judgment_count or 0), "productJudgmentPackageCount": int(package_count or 0), "taskDecisionCount": int(task_decision_count or 0), "inputBundleCount": int(bundle_count or 0), "frontendTaskViewCount": int(frontend_task_count or 0), "runtimeResidual": residual, "staleRunIgnored": stale_run_only, "stations": stations, "latestRun": latest_run, "decisionCounts": decision_counts, "updatedAt": now_iso(), "rule": "V14.9.4 metro line shows current-run counts and API budget: judgments may be high, Agent1 API calls must be <= bundle count."}
