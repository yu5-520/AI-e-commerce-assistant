...
IMPLEMENTATION_SEQUENCE = [
    ...
    "下一步：PostgreSQL 主写切换前检查清单，或 README / docs 进一步拆分",
]


def p0_architecture_summary(ctx: UserContext) -> dict[str, Any]:
    query_plan = query_plan_for_context(ctx, table_alias="resource")
    return {"version": P0_ARCHITECTURE_VERSION, "title": "互联网大厂 SaaS P0 架构拆解", "runtimeMode": "mirror_base_refactored", "currentContext": ctx.to_dict(), "mandatoryScopePlan": {"where": query_plan.where, "params": query_plan.params, "rule": query_plan.rule}, "taskPersistence": task_persistence_summary(), "layers": P0_LAYERS, "implementationSequence": IMPLEMENTATION_SEQUENCE, "definitionOfDone": ["业务查询按 tenant_id + deleted_at + Data Scope 过滤。", "任务、导入、投影、数据版本、预警、队列、审计、日志均可在 hybrid/postgres 模式 mirror。", "mirror 控制流统一由 repository_mirror_base_service 承接。"]}
