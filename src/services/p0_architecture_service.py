...
P0_LAYERS: list[dict[str, Any]] = [
    ...
    {"id": "P0-4", "name": "报表导入", "status": "projection_job_hybrid_mirror", "target": "import_jobs / projection_jobs", "currentGap": "ImportJob 与 ProjectionJob 已支持 SQLite-first mirror。"},
    {"id": "P0-5", "name": "PostgreSQL 数据模型", "status": "data_version_alert_event_models", "target": "PostgreSQL + Alembic", "currentGap": "DataVersion / AlertEvent 模型和迁移已补齐。"},
...
IMPLEMENTATION_SEQUENCE = [
    ...
    "DataVersion / AlertEvent 生产模型已补齐",
    "下一步：DataVersion / AlertEvent 写路径 mirror，或前端系统状态页",
]


def p0_architecture_summary(ctx: UserContext) -> dict[str, Any]:
    query_plan = query_plan_for_context(ctx, table_alias="resource")
    return {"version": P0_ARCHITECTURE_VERSION, "title": "互联网大厂 SaaS P0 架构拆解", "runtimeMode": "projection_data_hybrid_mirror", "currentContext": ctx.to_dict(), "mandatoryScopePlan": {"where": query_plan.where, "params": query_plan.params, "rule": query_plan.rule}, "taskPersistence": task_persistence_summary(), "layers": P0_LAYERS, "implementationSequence": IMPLEMENTATION_SEQUENCE, "definitionOfDone": ["业务查询按 tenant_id + deleted_at + Data Scope 过滤。", "任务、导入、队列、审计、日志、投影可在 hybrid/postgres 模式 mirror。", "DataVersion / AlertEvent 已进入生产模型。"]}
