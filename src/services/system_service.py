...
def _ensure_runtime_tables() -> None:
    ...
    try:
        ...
        pass
    try:
        from src.services.agent_budget_ledger_service import ensure_agent_budget_tables
...
def _runtime_dirty_diagnostics(table_status: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {"version": SYSTEM_SERVICE_VERSION, "status": "failed" if dirty else "passed", "runtimeDirty": dirty, "factSourceCount": fact_count, "v15ResidualCount": residual_count, "v15ResidualTables": {name: counts.get(name, 0) for name in V15_RUNTIME_TABLES}, "rule": "If fact sources are empty, V15 snapshots/judgments/packages/runs/Agent ledgers/read-models must also be empty."}
...
def clear_runtime_data(include_audit_logs: bool = True, *, reason: str = "manual_reset") -> Dict[str, Any]:
    ...
    return {"ok": True, "version": SYSTEM_SERVICE_VERSION, "message": "演示运行态已全链路清空：导入、经营对象、指标事实、V14/V15快照、信号池、双Agent判断、商品判断包、任务生成决策、Agent预算账本、报表schema缓存、任务生成快照、任务池、前端读模型和日志均回到空状态。", "reason": reason, "deletedTables": deleted_tables, "removedFiles": removed_files, "includeAuditLogs": include_audit_logs, "boundary": RUNTIME_BOUNDARY_NOTE, "db_status": get_db_status()}
...
def reset_legacy_runtime_once() -> Dict[str, Any]:
    ...
    with connect() as conn:
        if any(_meta_get(conn, marker) == "done" for marker in RESET_MARKERS):
            return {"ok": True, "skipped": True, "marker": V15_RUNTIME_RESET_MARKER, "message": "Runtime cleanup already applied."}
    ...
    if stale_count <= 0:
        with connect() as conn:
            ...
            conn.commit()
        reset_tasks()
        return {"ok": True, "skipped": True, "marker": V15_RUNTIME_RESET_MARKER, "staleRecordCount": 0, "message": "Runtime already empty; cleanup marker recorded."}
    result = clear_runtime_data(include_audit_logs=False, reason="v15_startup_one_time_agent_budget_runtime_cleanup")
    result["staleRecordCount"] = stale_count
    result["marker"] = V15_RUNTIME_RESET_MARKER
    return result
