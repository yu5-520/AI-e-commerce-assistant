...
RUNTIME_BOUNDARY_NOTE = "清空演示环境会删除报表导入、快照、业务信号、任务、日志、导入生成的经营对象主档、指标事实、数据缺口、经营节奏信号、经营页快照、pipeline阶段阀门、V13.1快照到任务判断交接记录和运维火车巡检记录；账号、角色、权限和基础店铺配置保留。"
...
def clear_runtime_data(include_audit_logs: bool = True, *, reason: str = "manual_reset") -> Dict[str, Any]:
    ...
    with connect() as conn:
        ...
        for marker in [V5_RUNTIME_RESET_MARKER, V1114_RUNTIME_RESET_MARKER, V121_RUNTIME_RESET_MARKER, V1213_RUNTIME_RESET_MARKER, V124_RUNTIME_RESET_MARKER, V12131_RUNTIME_RESET_MARKER, V12140_RUNTIME_RESET_MARKER, V131_RUNTIME_RESET_MARKER]:
    ...
    return {"ok": True, "message": "演示运行态已全链路清空：导入行、快照、pipeline阀门、快照到任务判断交接、运维火车巡检记录、业务信号、任务、日志、经营商品、经营店铺、指标事实、数据缺口和经营节奏信号均回到空状态。", "reason": reason, "deletedTables": deleted_tables, "removedFiles": removed_files, "includeAuditLogs": include_audit_logs, "boundary": RUNTIME_BOUNDARY_NOTE, "db_status": get_db_status()}
...
def reset_legacy_runtime_once() -> Dict[str, Any]:
    init_db()
    with connect() as conn:
        if any(_meta_get(conn, marker) == "done" for marker in [V131_RUNTIME_RESET_MARKER, V12140_RUNTIME_RESET_MARKER, V12131_RUNTIME_RESET_MARKER, V124_RUNTIME_RESET_MARKER, V1213_RUNTIME_RESET_MARKER, V121_RUNTIME_RESET_MARKER, V1114_RUNTIME_RESET_MARKER, V5_RUNTIME_RESET_MARKER]):
            return {"ok": True, "skipped": True, "marker": V131_RUNTIME_RESET_MARKER, "message": "Runtime cleanup already applied."}
    ...
    if stale_count <= 0:
        with connect() as conn:
            ...
            conn.commit()
        reset_tasks()
        return {"ok": True, "skipped": True, "marker": V131_RUNTIME_RESET_MARKER, "staleRecordCount": 0, "message": "Runtime already empty; cleanup marker recorded."}
    result = clear_runtime_data(include_audit_logs=False, reason="v131_startup_one_time_full_runtime_cleanup")
    result["staleRecordCount"] = stale_count
    result["marker"] = V131_RUNTIME_RESET_MARKER
    return result
