...
def list_station_contracts() -> Dict[str, Any]:
    return {
        "version": STATION_CONTRACT_VERSION,
        "contracts": [station_contract(station["stationId"]) for station in list_stations()],
        "rule": "前后端统一读取 Station Contract，不直接依赖站点内部实现。",
    }
...
def station_health(station_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        module_ok = True
    except Exception as exc:  # pragma: no cover - health surface only
        error = str(exc)
    ...
    return {
        ...
        "rule": "Health 只检查标准站点接口和模块可达性，不执行业务数据流。",
    }
...
def run_station_contract(station_id: str, body: Dict[str, Any] | None = None, *, diagnostic: bool = False) -> Dict[str, Any]:
    ...
    return {
        ...
        "gate": gate,
        "nextStation": station.get("nextStation"),
        "rule": "V12.14 标准站点运行只通过 Station Interface 写阀门和输出引用。",
    }
