...
IMPORT_JOB_WORKER_VERSION = "5.2.3"
...
def enqueue_import_worker_job(ctx: UserContext, *, dataset_name: str, source_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    worker_payload = {
        ...
        "payload": {"version": IMPORT_JOB_WORKER_VERSION, "datasetName": dataset_name, "sourceType": source_type, "body": payload},
    }
    ...
    dispatch = dispatch_arq_or_fallback(
        ctx,
        "import_report",
        {"workerJobId": worker_job.get("workerJobId"), "workerId": "arq-import-worker", "queueName": IMPORT_QUEUE_NAME, "sourceType": source_type, "datasetName": dataset_name},
    )
    return {
        ...
        "arqDispatch": dispatch,
        "rule": "enqueue=true writes worker_jobs first; Redis / ARQ dispatch is optional and falls back to SQLite.",
    }
...
def execute_next_import_worker_job(ctx: UserContext, *, worker_id: str = "import-worker-demo") -> Dict[str, Any]:
    ...
    try:
        ...
        return {"version": IMPORT_JOB_WORKER_VERSION, "mode": "import_worker_execute_next", "workerJob": completed.get("job") if completed else job, "importResult": result}
    except Exception as exc:  # noqa: BLE001
        failed = fail_worker_job(ctx, job["workerJobId"], error_message=str(exc), retry=True)
        return {"version": IMPORT_JOB_WORKER_VERSION, "mode": "import_worker_execute_next", "workerJob": failed.get("job") if failed else job, "error": str(exc)}
