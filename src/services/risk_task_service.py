...
- V12.4 upload-frequency + 3/7/14/30/90 day operating cadence tasks;
...
def generate_risk_tasks_for_signals(data_version: str | None = None, limit: int = 200, requester_role_id: str = "operator") -> Dict[str, Any]:
    ...
    operating tasks or report seeds when upload cadence and trend windows show
    meaningful business movement.
    """
    ...
    return {
        **risk_result,
        ...
        "mode": "v12_4_redline_plus_operating_cadence_agent_task_generation",
        ...
        "rule": "V12.4：红线继续硬控；非红线由上传频率、趋势周期和Agent经营判断生成日常经营任务或日报/周报候选。",
    }
...
def risk_task_summary(limit: int = 30) -> Dict[str, Any]:
    ...
    summary["rule"] = "V12.4：任务池、候选任务、趋势信号和观察项共同支撑日报/周报；不是只有硬基线任务才进入报告。"
    return summary
