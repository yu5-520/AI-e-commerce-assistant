"""V14.1 Task Snapshot Station service."""
...
TASK_SNAPSHOT_STATION_VERSION = "14.1.0"
...
def _normalize_decision(value: str | None) -> str:
    ...
    aliases = {"create_task": "create_task_snapshot", "create": "create_task_snapshot", "task": "create_task_snapshot", "manager_review": "manager_review_required", "review_required": "manager_review_required", "observe": "observe_only", "observation": "observe_only", "ignore": "ignore_noise", "noise": "ignore_noise"}
...
def create_task_snapshot(body: Dict[str, Any] | None = None, *, created_by: str | None = None, force: bool = False) -> Dict[str, Any]:
    ...
    snapshot["rule"] = "V14.1 snapshot package; pool entry is handled by task_pool_station."
    return snapshot
...
def task_snapshot_summary(limit: int = 50) -> Dict[str, Any]:
    ...
    return {"version": TASK_SNAPSHOT_STATION_VERSION, "total": len(items), "byDecision": by_decision, "byStatus": by_status, "latest": items[0] if items else None, "items": items, "rule": "V14.1 snapshots are idempotent by signalRef + decision + dataVersion."}
