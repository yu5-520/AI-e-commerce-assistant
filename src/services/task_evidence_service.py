...
DOMAIN_TEMPLATES: Dict[str, Dict[str, Any]] = {
    "库存": {
        "title": "库存处理表单",
        "fields": ["当前库存确认", "预计补货时间", "是否暂停投流", "是否调整主推商品", "是否需要总管协调供货"],
        "actions": ["确认库存", "确认补货", "暂停投流", "调整主推"],
    },
    "售后": {
        "title": "售后处理表单",
        "fields": ["主要退款原因", "是否商品问题", "是否详情页承诺问题", "是否客服话术问题", "是否暂停放量"],
        "actions": ["复查退款", "修改承诺", "调整话术", "暂停放量"],
    },
    "流量": {
        "title": "流量处理表单",
        "fields": ["当前 ROI", "是否继续投放", "是否更换素材", "是否降低预算", "是否先查商品承接"],
        "actions": ["降低预算", "更换素材", "暂停投放", "继续观察"],
    },
    "价格": {
        "title": "价格 / 毛利处理表单",
        "fields": ["当前售价", "当前成本", "活动价是否异常", "是否需要调价", "是否暂停活动"],
        "actions": ["核对成本", "核对活动价", "暂停活动", "提交财务复核"],
    },
    "报表": {
        "title": "报表处理表单",
        "fields": ["数据是否可信", "是否需要重新导入", "异常字段", "影响范围", "复盘说明"],
        "actions": ["确认数据", "重新导入", "补充说明", "提交复核"],
    },
}
...
def review_task_evidence(task_id: str, body: Dict[str, Any] | None = None, reviewer_id: str | None = None) -> Dict[str, Any] | None:
    ...
    task.update({
        "evidenceReviews": reviews[:10],
        "latestEvidenceReview": review,
