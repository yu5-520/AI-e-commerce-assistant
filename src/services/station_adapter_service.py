"""V15 station adapter service."""

from __future__ import annotations

from typing import Any, Dict

STATION_ADAPTER_VERSION = "15.0"
...
def simulated_station_output(station: Dict[str, Any], body: Dict[str, Any] | None = None, *, diagnostic: bool = False) -> Dict[str, Any]:
    body = body or {}
    data_version = body.get("dataVersion") or body.get("data_version") or ("DIAG-V15" if diagnostic else None)
...
def run_station_adapter(station: Dict[str, Any], body: Dict[str, Any] | None = None, *, diagnostic: bool = False) -> Dict[str, Any]:
    ...
    if station_id == "product_signal_snapshot_station":
        ...
        output = {"version": STATION_ADAPTER_VERSION, "adapterMode": "real_full_product_bundle_snapshot", "stationId": station_id, "stage": station.get("stage"), "dataVersion": data_version, "productSnapshotCount": result.get("productSnapshotCount", 0), "productSignalPackageCount": result.get("productSignalPackageCount", result.get("productSignalCount", 0)), "productSignalCount": result.get("productSignalCount", 0), "productSignalSnapshotRef": result.get("productSignalSnapshotRef"), "outputRef": result.get("outputRef") or f"product_signal_snapshot:{data_version or 'latest'}", "productSignalSnapshot": result, "isDiagnostic": False, "rule": "V15 outputs fullProductBundle and refreshes frontend_product_view."}
    ...
    if station_id == "rag_context_station":
        ...
        return {"version": STATION_ADAPTER_VERSION, "adapterMode": "real_rag_volatility_context", "stationId": station_id, "stage": station.get("stage"), "dataVersion": data_version, "matchedContextCount": result.get("matchedContextCount", 0), "ragContextRef": result.get("ragContextRef"), "outputRef": result.get("outputRef") or f"rag_context:{data_version or 'latest'}", "ragContext": result, "isDiagnostic": False, "rule": "RAG supplies volatility context; calls must be ledger counted."}

    if station_id == "agent_judgment_station":
        from src.services.dual_agent_product_task_service import run_dual_agent_product_task_pipeline
