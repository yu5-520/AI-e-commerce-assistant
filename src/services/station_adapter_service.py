"""V15.1 station adapter service."""

from __future__ import annotations

from typing import Any, Dict

STATION_ADAPTER_VERSION = "15.1"
...
def simulated_station_output(station: Dict[str, Any], body: Dict[str, Any] | None = None, *, diagnostic: bool = False) -> Dict[str, Any]:
    body = body or {}
    data_version = body.get("dataVersion") or body.get("data_version") or ("DIAG-V15.1" if diagnostic else None)
...
def run_station_adapter(station: Dict[str, Any], body: Dict[str, Any] | None = None, *, diagnostic: bool = False) -> Dict[str, Any]:
    ...
    if station_id == "product_signal_snapshot_station":
        ...
        output = {"version": STATION_ADAPTER_VERSION, "adapterMode": "real_full_product_bundle_snapshot", "stationId": station_id, "stage": station.get("stage"), "dataVersion": data_version, "productSnapshotCount": result.get("productSnapshotCount", 0), "productSignalPackageCount": result.get("productSignalPackageCount", result.get("productSignalCount", 0)), "productSignalCount": result.get("productSignalCount", 0), "productSignalSnapshotRef": result.get("productSignalSnapshotRef"), "outputRef": result.get("outputRef") or f"product_signal_snapshot:{data_version or 'latest'}", "productSignalSnapshot": result, "isDiagnostic": False, "rule": "V15.1 outputs current-run fullProductBundle and refreshes frontend_product_view."}
    ...
    if station_id == "rag_context_station":
        ...
        return {"version": STATION_ADAPTER_VERSION, "adapterMode": "real_rag_volatility_context", "stationId": station_id, "stage": station.get("stage"), "dataVersion": data_version, "matchedContextCount": result.get("matchedContextCount", 0), "ragContextRef": result.get("ragContextRef"), "outputRef": result.get("outputRef") or f"rag_context:{data_version or 'latest'}", "ragContext": result, "isDiagnostic": False, "rule": "RAG supplies volatility context; calls are ledger counted."}

    if station_id == "agent_judgment_station":
        from src.services.dual_agent_product_task_v151_service import run_dual_agent_product_task_pipeline
        ...
        output = {"version": STATION_ADAPTER_VERSION, "adapterMode": "real_v151_current_run_data_change_agent_budget_pipeline", "stationId": station_id, "stage": station.get("stage"), "dataVersion": data_version, "decision": "mixed" if result.get("judgmentCount") else "no_bundles", "confidence": 0.72 if result.get("productJudgmentPackageCount") else 0, "judgmentCount": result.get("judgmentCount", 0), "rawJudgmentCount": result.get("rawJudgmentCount", 0), "averageJudgmentsPerSignal": result.get("averageJudgmentsPerSignal", 0), "metricJudgmentMode": result.get("metricJudgmentMode"), "agent1ApiMode": result.get("agent1ApiMode"), "agent1ApiCallCount": result.get("agent1ApiCallCount", 0), "taskMappingApiCallCount": result.get("taskMappingApiCallCount", 0), "totalAgentCallCount": result.get("totalAgentCallCount", 0), "totalAgentBudget": result.get("totalAgentBudget", 8), "apiBudgetViolation": result.get("apiBudgetViolation", False), "agentBudgetLedger": result.get("agentBudgetLedger"), "ragRetrievalCount": result.get("ragRetrievalCount", 0), "productJudgmentPackageCount": result.get("productJudgmentPackageCount", 0), "identityGapCount": result.get("identityGapCount", 0), "taskDecisionCount": result.get("taskDecisionCount", 0), "formalDecisionCount": result.get("formalDecisionCount", 0), "streamedTaskSnapshotCount": result.get("streamedTaskSnapshotCount", 0), "streamedTaskPoolCount": result.get("streamedTaskPoolCount", 0), "taskGenerationRun": result.get("taskGenerationRun"), "agentJudgmentRef": result.get("agentJudgmentRef"), "outputRef": result.get("outputRef") or f"dual_agent_product_task:{data_version or 'latest'}", "agentJudgment": result, "isDiagnostic": False, "rule": "V15.1: product judgments follow real fieldSignals; task read model follows current dataVersion."}
        output["readModelRefresh"] = _refresh_read_model(station_id, data_version, output)
        return output
