"""V16.1 station adapter service."""

from __future__ import annotations

from typing import Any, Dict

STATION_ADAPTER_VERSION = "16.1"
...
def simulated_station_output(station: Dict[str, Any], body: Dict[str, Any] | None = None, *, diagnostic: bool = False) -> Dict[str, Any]:
    body = body or {}
    data_version = body.get("dataVersion") or body.get("data_version") or ("DIAG-V16.1" if diagnostic else None)
...
def run_station_adapter(station: Dict[str, Any], body: Dict[str, Any] | None = None, *, diagnostic: bool = False) -> Dict[str, Any]:
    ...
    if station_id == "product_signal_snapshot_station":
        ...
        output = {"version": STATION_ADAPTER_VERSION, "adapterMode": "real_full_product_bundle_snapshot", "stationId": station_id, "stage": station.get("stage"), "dataVersion": data_version, "productSnapshotCount": result.get("productSnapshotCount", 0), "productSignalPackageCount": result.get("productSignalPackageCount", result.get("productSignalCount", 0)), "productSignalCount": result.get("productSignalCount", 0), "productSignalSnapshotRef": result.get("productSignalSnapshotRef"), "outputRef": result.get("outputRef") or f"product_signal_snapshot:{data_version or 'latest'}", "productSignalSnapshot": result, "isDiagnostic": False, "rule": "V16.1 outputs current-run fullProductBundle and refreshes frontend_product_view."}
    ...
    if station_id == "rag_context_station":
        ...
        return {"version": STATION_ADAPTER_VERSION, "adapterMode": "real_rag_volatility_context", "stationId": station_id, "stage": station.get("stage"), "dataVersion": data_version, "matchedContextCount": result.get("matchedContextCount", 0), "ragContextRef": result.get("ragContextRef"), "outputRef": result.get("outputRef") or f"rag_context:{data_version or 'latest'}", "ragContext": result, "isDiagnostic": False, "rule": "RAG supplies product judgment context; retrieval count is budget-ledger counted."}

    if station_id == "agent_judgment_station":
        from src.services.real_product_judgment_agent_v161_service import run_dual_agent_product_task_pipeline
    ...
    if station_id == "task_pool_station":
        ...
        output = {"version": STATION_ADAPTER_VERSION, "adapterMode": "real_task_pool_from_soft_routed_sop_snapshots", "stationId": station_id, "stage": station.get("stage"), "dataVersion": data_version or result.get("dataVersion"), "poolEntryId": latest_entry.get("poolEntryId"), "taskId": latest_entry.get("taskId"), "createdTaskCount": result.get("createdTaskCount", 0), "outputRef": f"task_pool:{latest_entry.get('poolEntryId') or data_version or 'latest'}", "taskPool": result, "isDiagnostic": False, "rule": "task pool consumes product-level SOP snapshots and refreshes current-run read model"}
        output["readModelRefresh"] = _refresh_read_model(station_id, data_version, output)
        return output
