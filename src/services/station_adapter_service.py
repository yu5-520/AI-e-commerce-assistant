"""V16.2 station adapter service."""

from __future__ import annotations

from typing import Any, Dict

STATION_ADAPTER_VERSION = "16.2"
...
def simulated_station_output(station: Dict[str, Any], body: Dict[str, Any] | None = None, *, diagnostic: bool = False) -> Dict[str, Any]:
    body = body or {}
    data_version = body.get("dataVersion") or body.get("data_version") or ("DIAG-V16.2" if diagnostic else None)
...
def run_station_adapter(station: Dict[str, Any], body: Dict[str, Any] | None = None, *, diagnostic: bool = False) -> Dict[str, Any]:
    ...
    if station_id == "product_signal_snapshot_station":
        ...
        output = {"version": STATION_ADAPTER_VERSION, "adapterMode": "real_full_product_bundle_snapshot", "stationId": station_id, "stage": station.get("stage"), "dataVersion": data_version, "productSnapshotCount": result.get("productSnapshotCount", 0), "productSignalPackageCount": result.get("productSignalPackageCount", result.get("productSignalCount", 0)), "productSignalCount": result.get("productSignalCount", 0), "productSignalSnapshotRef": result.get("productSignalSnapshotRef"), "outputRef": result.get("outputRef") or f"product_signal_snapshot:{data_version or 'latest'}", "productSignalSnapshot": result, "isDiagnostic": False, "rule": "V16.2 outputs current-run fullProductBundle and refreshes frontend_product_view."}
