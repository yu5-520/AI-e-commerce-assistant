...
USER_ACTIONS_BY_ROLE: Dict[str, Any] = {"owner": ["view", "follow", "confirm"], "manager": ["dispatch", "approve", "reject"], "operator": ["accept", "submit", "supplement"]}
...
FRONTEND_LAYOUT_RULES = {"titleArea": "compact", "explanationText": "minimized", "mainActionArea": "dominant", "dataFlow": "collapsed_by_default", "systemInfo": "system_page_only", "taskCard": "primary_surface", "visualRatio": "main_70_aux_20_title_10", "firstScreenGoal": "user_knows_next_action"}
V102_UI_PRODUCTIZATION_RULES = ["topbar and page hero stay compact", "main action and task cards occupy the strongest visual weight", "data flow is summarized in one line or collapsed into details", "system status and version detail do not compete with daily operation", "each page keeps a clear primary action above secondary data"]
...
V103_DASHBOARD_RULES = ["dashboard answers what should be handled today", "priority tasks are shown before metrics", "high risk, report result, review and progress are secondary support blocks", "dashboard must route users into tasks instead of making them browse modules", "dashboard is a workbench, not a menu wall or architecture status page"]
...
V104_IMPORT_REFRESH_CONTRACT = {"updatedModules": ["dashboard", "operation", "tasks", "reports", "logs"], "frontendRefreshTargets": ["dashboard", "operating-unit", "business-actions", "data-check", "business-report"], "userMessage": "已更新，生成 X 个任务", "dataFlowDisplay": "collapsed_status_line"}
...
V105_ROLE_VIEW_RULES = {"owner": {"surface": "progress", "actions": ["view", "follow", "confirm"]}, "manager": {"surface": "dispatch_review", "actions": ["dispatch", "approve", "reject"]}, "operator": {"surface": "execution", "actions": ["accept", "submit", "supplement"]}}
V106_TASK_ACTION_RULES = ["one_primary_action_per_task_card", "one_secondary_action_allowed", "owner_view_follow_confirm", "manager_dispatch_approve_reject", "operator_accept_submit_supplement", "backend_keeps_full_events_and_logs"]
V107_OPERATING_PROFILE_RULES = ["agent_assigns_tags_without_user_confirmation", "tags_are_agent_working_language", "user_can_edit_but_does_not_confirm_by_default", "operating_profile_is_created_after_store_and_product_import", "tag_drift_becomes_task_when_business_data_keeps_declining", "agent_reads_profile_before_generating_tasks"]
V107_TAG_TYPES = ["vertical_category", "store_weight", "product_role", "risk", "task_intensity"]
V107_PROFILE_SURFACES = ["store_profile", "product_profile", "agent_task_context", "tag_change_task_candidate"]
V108_TAG_CHANGE_TASK_RULES = ["tag_change_candidates_become_tasks", "user_intervention_only_through_tasks", "agent_profile_snapshot_is_attached_to_task", "manager_operator_owner_views_reuse_v10_5_and_v10_6", "task_result_can_enter_logs_and_rag_memory_candidate"]
V108_TAG_CHANGE_TASK_FLOW = ["profile_tag_drift_detected", "tag_change_candidate_created", "task_written_to_task_pool", "role_views_synced", "user_handles_task", "logs_and_rag_candidate_written"]
...
def task_driven_product_summary(ctx: UserContext) -> Dict[str, Any]:
    ...
    return {
        ...
        "operatingProfileRules": V107_OPERATING_PROFILE_RULES,
        "operatingProfileTagTypes": V107_TAG_TYPES,
