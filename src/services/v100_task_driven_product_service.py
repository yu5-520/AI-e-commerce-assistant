"""V10.7 task-driven product contract."""
...
V100_TASK_PRODUCT_VERSION = "10.7.0"
...
def task_driven_product_summary(ctx: UserContext) -> Dict[str, Any]:
    return {
        ...
        "nonGoals": ["不让用户默认确认标签", "不让用户手动维护分类作为主流程", "不把复杂流程节点暴露到经营界面", "不让用户手动选择跨账号流程节点", "不让任务卡堆满低频按钮", "不把标签确认变成用户必做动作"],
        "context": ctx.to_dict(),
