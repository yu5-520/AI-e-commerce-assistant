"""Generate Markdown and JSON reports for the active operating-unit workflow."""
...
def write_markdown_report(
    ...
    """Write the current ERP operating-unit report."""
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    path = OUTPUT_DIR / "operating_report.md"
    ...
    lines.extend(
        [
            ...
            "## 12. 复盘结论",
            "",
            "当前主线已跑通：Mock ERP / CRM 数据导入 → ERP 经营单元识别 → 循环频率策略 → 类目上下文加载 → 商品体检 → 同经营单元竞品机会 → 同经营单元上新建议 → 流量测试回流 → 经营循环总控 → RAG 召回 → 待确认动作 → 人工确认边界 → 经营报告输出。",
        ]
    )
