...
> 当前版本：V9.1.0。V9.1 是仓库结构一致性版本：在 V9.0 SaaS 企业级一致性底座上，进一步统一目录职责、文档入口、脚本职责、CI 检查和前端缓存版本，防止旧 Demo 入口、旧文档和旧路径回流。
...
核心规则：**前端模块保持稳定；后端能力统一整合；套餐能力按层隔离；RAG 与数据按命名空间隔离；关键动作必须审批、留痕、可追溯；仓库入口、文档、脚本和 CI 必须指向同一主线。**
