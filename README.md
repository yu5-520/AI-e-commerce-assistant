...
> 当前版本：V5.0.8。总览已从技术状态页改成经营首页：隐藏完整 DataVersion 工程代号，展示最新导入、报表记录数、商品数、任务数和按紧急程度 / 截止时间排序的当前任务。
...
核心规则：**首页展示经营摘要，不展示工程代号；报表确认导入就是自动入库；商品、报表、总览必须随导入同步刷新；当前任务按优先级、截止时间和风险域排序。**

## 关键目录

```text
src/api/main.py                              FastAPI 入口，版本 5.0.8
...
web_demo/index.html                           前端入口，缓存号 v5.0.8
