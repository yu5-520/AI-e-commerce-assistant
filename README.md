...
当前基线：**V15.1 本轮数据隔离 + 去 DEMO 垫底污染版**。

V15.1 保留 V15 的三阶段 Agent 预算账本，同时补上两个生产级硬闸：第一，商品判断不再固定把每个商品补齐成 8 条指标判断，而是只根据 fullProductBundle 中真实变化、异常和关键数据缺口生成判断；第二，任务页只读取最新 dataVersion 的任务，旧 DEMO、seed、历史任务不能污染当前执行队列。
...
→ 商品判断 Agent：只把真实变化、异常、关键数据缺口展开成指标判断
...
商品判断数量必须由真实 fieldSignals / abnormal metrics / data gaps 决定，不能固定按 8 条指标补齐。
...
frontend_task_view 只从当前 dataVersion 的 task_pool_entries 重建。
