...
> 当前版本：V5.2.1。新增 ImportJob 入队执行模式：`/api/data/import-jobs/confirm`、`/report`、`/mock-alerts` 支持 `enqueue: true`，只返回 WorkerJob；`/api/data/import-jobs/worker/execute-next` 可在 Demo 环境认领并执行下一条导入队列任务。
...
↓
Demo Worker：认领并执行下一条 import 队列任务
↓
...
src/api/routes/worker_jobs.py                  /api/worker/jobs/*
...
当前 P0 进度：**任务系统已具备 SQLite mirror、TaskRepository scoped reads、启动快照恢复、写路径过渡 API，并已接入 Agent 入池、待办核心生命周期动作、报表导入前端同步、创意 Agent 入池和证据提交审计入库。报表导入已新增 ImportJob / ProjectionJob 运行记录，Worker Queue 已支持入队与重试，ImportJob 已支持 enqueue=true 和 Demo Worker 执行下一条导入任务。下一步是 Redis / ARQ 常驻 Worker 替换。**

## 关键目录

```text
src/api/main.py                                 FastAPI 入口，版本 5.2.1
...
src/api/routes/worker_jobs.py                   Worker 队列 API
...
15. Redis / ARQ 替换：把 SQLite 队列表切换为真正后台 Worker 执行
16. LLM Gateway：熔断、限流、配额、缓存、Schema 校验、Trace、降级模板
17. Audit / Logs：业务审计表 + JSON 技术日志 + trace_id
18. Nginx：前后端分离、HTTPS、限流、安全头
```
