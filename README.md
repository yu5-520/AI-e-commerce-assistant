...
> 当前版本：V5.1.6。新增前端报表导入自动同步：保留原报表导入接口不动，通过 `web_demo/core/report-task-sync.js` 包装 `confirmReportImport` 与 `importMockAlerts`，导入成功后自动调用 `/api/data/report-tasks/sync-current`，把报表预警任务同步进 TaskRepository。
...
当前 P0 进度：**任务系统已具备 SQLite mirror、TaskRepository scoped reads、启动快照恢复、写路径过渡 API，并已接入 Agent 入池与待办核心生命周期动作。报表导入确认和示例试跑已通过前端补丁自动同步报表任务到 TaskRepository。下一步是创意 Agent 入池、证据提交审计、导入服务本体 ImportJob 化。**

## 关键目录

```text
src/api/main.py                                 FastAPI 入口，版本 5.1.6
...
src/api/routes/data_import.py                   旧报表导入接口；当前由前端补丁接同步桥
...
web_demo/index.html                             前端入口，已加载 report-task-sync.js
...
10. 剩余任务入口切换：创意 Agent 入池、证据提交审计、导入服务本体 ImportJob 化
11. ImportJob：报表导入、DataVersion、ImportedRows、ProjectionJob、AlertEvent 串链
12. Worker / Redis：导入、投影、预警、Agent 异步化与幂等重试
13. LLM Gateway：熔断、限流、配额、缓存、Schema 校验、Trace、降级模板
14. Audit / Logs：业务审计表 + JSON 技术日志 + trace_id
15. Nginx：前后端分离、HTTPS、限流、安全头
```
