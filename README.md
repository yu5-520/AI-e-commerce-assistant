...
> 当前版本：V5.2.2。新增 Redis / ARQ Worker 配置骨架：新增 `worker_runtime_config_service.py`、`src/workers/task_registry.py`、`src/workers/arq_worker.py` 和 `/api/worker/jobs/runtime`；未配置 Redis 时继续走 SQLite worker_jobs fallback。
...
↓
WorkerJob：记录可异步执行的导入 / 投影 / 预警 / Agent 任务
↓
Worker Runtime：Redis / ARQ 可接管执行；未配置时回退 SQLite 队列表
↓
...
src/services/import_job_worker_service.py      ImportJob 入队与 Demo Worker 执行桥
...
src/api/routes/worker_jobs.py                  /api/worker/jobs/* 与 /api/worker/jobs/runtime
...
当前 P0 进度：**任务系统已具备 SQLite mirror、TaskRepository scoped reads、启动快照恢复、写路径过渡 API，并已接入 Agent 入池、待办核心生命周期动作、报表导入前端同步、创意 Agent 入池和证据提交审计入库。报表导入已新增 ImportJob / ProjectionJob 运行记录，Worker Queue 已支持入队与重试，ImportJob 已支持 enqueue=true 和 Demo Worker 执行下一条导入任务。Redis / ARQ 配置层、任务注册表和 WorkerSettings 已补齐。下一步是 Redis / ARQ 常驻执行。**

## 关键目录

```text
src/api/main.py                                 FastAPI 入口，版本 5.2.2
...
src/services/import_job_worker_service.py       ImportJob 入队与 Demo Worker 执行
...
src/api/routes/worker_jobs.py                   Worker 队列与 Runtime API
...
16. Redis / ARQ 常驻执行：让 ImportJob 不再依赖手动 execute-next
17. LLM Gateway：熔断、限流、配额、缓存、Schema 校验、Trace、降级模板
18. Audit / Logs：业务审计表 + JSON 技术日志 + trace_id
19. Nginx：前后端分离、HTTPS、限流、安全头
```
