...
> 当前版本：V5.2.3。新增 ARQ Dispatch + SQLite fallback：`enqueue=true` 会先写 SQLite `worker_jobs` 保证审计和可回放，再按 Worker Runtime 尝试投递 `arq_dispatch(import_report, payload)`；Redis 不可用时不会中断业务，返回 `arqDispatch.activeBackend=sqlite_fallback`。
...
17. Worker 任务扩展：projection_refresh、alert_generation、agent_analysis、rag_memory_write
18. LLM Gateway：熔断、限流、配额、缓存、Schema 校验、Trace、降级模板
19. Audit / Logs：业务审计表 + JSON 技术日志 + trace_id
