...
V11.13 的重点是 Demo 快速部署：高频小改不再走 release 发布；新增 deploy_fast.sh，只做 fetch、reset、版本一致性检查、systemd 重启和 health 检查，不 clone release、不重建 venv、不默认 pip install。
...
Demo 高频小改使用 deploy_fast.sh。
阶段收口使用 deploy_atomic.sh。
客户环境再走完整严格发布。
```
