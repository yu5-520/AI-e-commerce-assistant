...
> 当前版本：V5.2.0。新增 Worker / Redis 异步化骨架：新增 `worker_jobs` 队列表、`worker_queue_service.py` 和 `/api/worker/jobs/*`，支持入队、认领、完成、失败、重试、幂等 key 和优先级；当前仍用 SQLite 队列表作为过渡，后续替换为 Redis / ARQ。
...
当前 P0 进度：**任务系统已具备 SQLite mirror、TaskRepository scoped reads、启动快照恢复、写路径过渡 API，并已接入 Agent 入池、待办核心生命周期动作、报表导入前端同步、创意 Agent 入池和证据提交审计入库。报表导入已新增 ImportJob / ProjectionJob 运行记录。Worker / Redis 已进入 SQLite 队列骨架阶段，下一步是把 ImportJob 真正改成入队执行。**

## 关键目录

```text
src/api/main.py                                 FastAPI 入口，版本 5.2.0
...
14. Redis / ARQ 替换：把 SQLite 队列表切换为真正后台 Worker 执行
