...
> 当前版本：V9.0.0。V9 不新增前端业务模块，不继续扩展权重算法；本版本把 V1-V8 已有能力收束为 SaaS 企业级一致性底座，统一仓库、前端、后端、套餐隔离、RAG 隔离、权限审计、部署模式和测试验收口径。
...
核心规则：**前端模块保持稳定；后端能力统一整合；套餐能力按层隔离；RAG 与数据按命名空间隔离；关键动作必须审批、留痕、可追溯。**
...
```text
仓库一致性：README / VERSION / CHANGELOG / docs / scripts 指向同一主线。
...
V9.0 当前目标：统一版本主线，建立 SaaS 企业级一致性底座文档和治理口径。
