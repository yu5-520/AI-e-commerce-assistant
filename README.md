...
> 当前版本：V9.3.0。V9.3 是前端模块一致性版本：在 V9.0 SaaS 企业级一致性底座、V9.1 仓库结构一致性和 V9.2 后端主流程一致性上，固定前端主模块稳定策略，后端权重、RAG、审批、执行回写和复盘能力只作为原模块增强字段进入产品体验。
...
核心规则：**前端模块保持稳定；后端能力统一整合；套餐能力按层隔离；RAG 与数据按命名空间隔离；关键动作必须审批、留痕、可追溯；仓库入口、文档、脚本、CI、后端主流程和前端模块边界必须指向同一主线。**
...
↓
V9.3 Frontend Module Guard：稳定前端主模块、套餐展示深度、禁止权重能力乱拆新主模块
```
...
已完成：V9.3 前端模块一致性契约、架构可视入口和 CI 检查入口。
