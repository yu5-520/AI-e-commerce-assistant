...
> V4.2：在 V4 模块 Agent 与 V4.1 RAG-ready 经验记忆层之上，新增“自动解析生成任务 Agent”和“任务解析运营方式 Agent”。系统用规则保证稳定触发，用 RAG 召回复核经验，用 Agent 生成任务草案和多打法 playbook，用人工确认守住经营边界。
...
```text
Agent 可以生成建议、草案、任务拆解、报表摘要、日报 / 周报初稿、任务候选和运营打法。
...
src/api/routes/modules/agents.py          V4 / V4.2 Agent API
