...
当前基线：**V12.8.1 任务生命周期闭环 + 前后端接口契约收口**。

V12.8.1 保留 V12.8 的完整生命周期：生成任务 → 接收任务 → 提交处理材料 → 主管复核 → 生成自动复盘周期 → 复盘结束进入RAG候选 → 人工审核后写入RAG → RAG增强下一次任务生成。本版重点修正前后端契约漂移：前端任务页不再二次聚合任务，直接信任后端真实聚合任务；API client 暴露 lifecycle summary 与 recap complete；当前执行文档只描述 V12.8.1 主链路，不再让 V12.3/V12.7 旧流程占据当前验收位置。
...
→ web_demo/modules/todo/page.js 只做展示和排序，不再前端二次聚合
→ 运营接收任务
→ 运营提交处理材料 / evidence
→ 主管复核材料
...
前端必须调用 /api/modules/todo/lifecycle/summary 查看生命周期统计。
前端必须调用 /api/modules/todo/{id}/recap/complete 完成复盘并生成RAG候选。
