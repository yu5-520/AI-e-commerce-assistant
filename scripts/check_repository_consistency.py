...
def main() -> None:
    ...
    for marker in ["当前版本：V9.4.0", "/api/modules", "/api/accounts", "/api/architecture/v9/tier-isolation", "docs/V9_TIER_ISOLATION_CONSISTENCY.md"]:
        must_have(readme, marker, "README")
    for marker in ["scripts/check_version_governance.py", "scripts/check_repository_consistency.py", "scripts/check_backend_flow_consistency.py", "scripts/check_frontend_module_consistency.py", "scripts/check_tier_isolation_consistency.py"]:
        must_have(workflow, marker, "workflow")
    must_have(index_html, f"?v={EXPECTED_VERSION}", "web_demo/index.html")
    print("Repository consistency check passed for V9.4.")
