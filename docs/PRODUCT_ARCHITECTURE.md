...
当前 V11.17 Demo 目标：把经营页的一店一行卡片继续向下打通到店铺商品档案，验证真实报表 / 接口数据进入系统后，能否按店铺作用域稳定展示商品档案、商品标签、店铺标签、趋势信号、少量真正需要人处理的执行任务、详情报告、复核留痕，并支持反复测试时的一键清空运行态。
...
商品档案顶部必须展示当前店铺名、平台、商品数、店铺标签、店铺权重和执行任务数。
