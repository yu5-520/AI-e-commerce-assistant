...
V12 的目标不是继续扩大任务数量，而是把报表上传从“商品壳入库”升级成“报表画像 → 系统编码 → 指标事实 → 任务证据闸门”。V12.1.0 将指标事实从 `payload.metricFacts` 兼容缓存升级为独立 SQLite 事实表。
...
- V12.1.1：导入确认按 `reportProfile.sheetProfiles` 明确分 Sheet 写入事实表。
