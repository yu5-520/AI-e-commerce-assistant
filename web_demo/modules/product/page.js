(function () {
  ...
  function apiError(error) {
    return `<section class="product-toolbar"><div><p class="eyebrow">PRODUCT ARCHIVE · V11.16</p><h2>商品档案</h2></div></section><section class="page-section"><div class="section-header"><h3>接口异常</h3><span class="status-badge">无本地兜底</span></div><p>后端接口没有返回可用数据，页面已停止展示本地模拟业务内容。</p><strong>当前页面接口 ${s(error?.message || error || "请求失败")}</strong></section>`;
  }
  ...
  window.ProductPage = {
    ...
    async render(ctx) {
      ...
      return `<section class="product-toolbar"><div><p class="eyebrow">PRODUCT ARCHIVE · V11.16</p><h2>${empty ? "暂无商品数据" : `商品档案${s(filterText)}`}</h2><p>这里展示清洗后的商品列表、商品标签和基线状态；只有高风险高时效事项才进入任务栏。</p></div>${storeFilter ? `<button type="button" data-clear-filter>全部商品</button>` : ""}</section>${notice ? AppShell.notice("操作结果", notice) : ""}<section class="page-section product-list-section"><div class="section-header"><h3>商品列表</h3><span class="status-badge">${rows.length} 个商品</span></div><div class="product-card-list">${empty ? `<article class="dashboard-empty">暂无商品数据，请在报表模块导入商品、库存、订单或退款数据。</article>` : rows.map(renderRow).join("")}</div></section>`;
    },
    mount(ctx) {
