(function () {
  ...
  function alertBadge(product) {
    ...
    return `<div class="product-number-cell danger"><span>报表预警</span><strong>${s(state.activeAlertCount)}</strong><small>${s(state.highestPriority || "待处理")}</small></div>`;
  }
  function alertPanel(product) {
    ...
    return `<section class="page-section product-detail-section"><div class="section-header"><h3>报表触发预警</h3><span class="status-badge danger">${s(latest.priority || "中")}</span></div><p>${s(latest.alertType || "报表预警")} · ${s(latest.dataVersion || "数据版本")}</p><ul>${evidence}</ul></section>`;
  }
  ...
  window.ProductPage = {
    ...
    render() {
      ...
      return `<section class="product-toolbar"><div><p class="eyebrow">PRODUCT LIST · V3.0</p><h2>商品经营列表</h2><p>按平台、店铺和状态查看商品；新报表导入后，商品卡片会显示报表触发预警。</p></div></section>${notice ? AppShell.notice("操作结果", notice) : ""}<section class="page-section product-list-section"><div class="section-header"><h3>商品列表</h3><span class="status-badge">${products().length} 个商品</span></div><div class="product-card-list">${products().map(renderRow).join("")}</div></section>`;
    },
    mount(ctx) {
