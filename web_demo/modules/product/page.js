(function () {
  ...
  function normalizeProduct(item = {}) {
    return {
      ...
      avgOrderValue: item.avgOrderValue || item.price || "—",
      ...
      organicVisitors: item.organicVisitors || "—",
      paidVisitors: item.paidVisitors || "—",
      afterSales: item.afterSales || item.afterSalesStatus || "正常",
      afterSalesLevel: item.afterSalesLevel || "good",
      suggestion: item.suggestion || item.reason || "商品档案展示定位与指标事实，任务SOP在任务详情页处理。",
      ...
      taskHistorySummary: item.taskHistorySummary || {},
  ...
  function taskButton(item) {
    ...
    return task ? `<button type="button" data-open-task="${s(task.id)}" class="ghost">查看任务</button><button type="button" data-task-report="${s(task.id)}">任务报告</button>` : `<button type="button" data-candidate-module="product" data-candidate-id="${s(item.id)}">任务证据</button>`;
  }
  function scopeHero(rows = []) {
    if (!currentScope?.fromStore) {
      return `<section class="product-archive-hero"><div><p class="eyebrow">PRODUCT ARCHIVE · V12.1.2</p><h2>商品档案</h2><p>展示当前账号可见的商品定位、系统编码和指标事实。</p></div><div class="product-scope-panel"><span>商品</span><strong>${rows.length}</strong><small>全局商品档案</small></div></section>`;
    }
    return `<section class="product-archive-hero scoped"><div><p class="eyebrow">STORE PRODUCT ARCHIVE · V12.1.2</p><h2>${s(currentScope.storeName || "店铺商品档案")}</h2><p>${s(currentScope.platform || "平台")} · 当前店铺 ${rows.length || currentScope.productCount || 0} 个商品 · 店铺级商品档案</p>${smallTags([currentScope.storeWeightTag, ...(currentScope.businessTags || [])])}</div><div class="product-scope-panel"><span>执行任务</span><strong>${s(currentScope.activeTaskCount || 0)}</strong><small>${s((currentScope.productRoleTags || [])[0] || "商品状态")}</small><button type="button" class="secondary" data-clear-filter>全部商品</button></div></section>`;
  }
  ...
  function renderPosition(product) {
    ...
    const rows = [
      ["系统店铺编码", pos.systemStoreCode],
      ["系统SPU编码", pos.systemSpuCode],
      ["系统LINK编码", pos.systemLinkCode],
      ["系统SKU编码", pos.systemSkuCode],
      ["平台", pos.platform || product.platform],
      ["店铺", pos.storeName || product.store],
      ["商品ID", pos.productId || product.productId],
      ["SKU ID", pos.skuId || product.skuId],
      ["ERP编码", pos.erpProductCode || product.erpProductCode],
      ["商品链接", pos.productLink || product.productLink || product.link],
  ...
  function renderTrafficFacts(product) {
    const rows = product.trafficSourceFacts || [];
    if (!rows.length) return "";
    return `<section class="page-section product-detail-section"><div class="section-header"><h3>流量来源</h3><span class="status-badge">${rows.length} 类</span></div><div class="product-traffic-list">${rows.map((item) => `<article><strong>${s(item.trafficSource)}</strong><span>访客 ${s(item.visitorCount || "—")}</span><span>点击率 ${s(item.clickRate || "—")}</span><span>转化 ${s(item.conversionRate || "—")}</span><span>ROI ${s(item.roi || "—")}</span></article>`).join("")}</div></section>`;
  }
  function renderTaskSummary(product) {
    const summary = product.taskHistorySummary || {};
    return `<section class="page-section product-detail-section"><div class="section-header"><h3>任务历史摘要</h3><span class="status-badge">${summary.hasActiveTask ? "有未完成任务" : "无未完成任务"}</span></div><p>${s(summary.summary || "商品页只显示任务摘要，完整SOP在任务详情页查看。")}</p><div class="product-position-grid compact"><div><span>任务次数</span><strong>${s(summary.taskCount || 0)}</strong></div><div><span>当前任务</span><strong>${s(summary.activeTaskStatus || "—")}</strong></div><div><span>最近完成</span><strong>${s(summary.completedTaskStatus || "—")}</strong></div></div></section>`;
  }
  function renderDetail(product) {
    return `<section class="product-detail-hero"><div class="product-detail-main"><div class="product-thumb large">${s(product.imageLabel || "品")}</div><div><p class="eyebrow">PRODUCT ARCHIVE · V12.1.2</p><h2>${s(product.title)}</h2><p>${s(product.platform)} · ${s(product.store)} · ${s(product.productId)}</p>${product.link || product.productLink ? `<a href="${s(product.link || product.productLink)}" target="_blank" rel="noreferrer">${s(product.link || product.productLink)}</a>` : ""}</div></div><div class="product-detail-actions"><button type="button" data-back>返回商品列表</button>${taskButton(product)}</div></section>${notice ? AppShell.notice("操作结果", notice) : ""}<section class="kpi-grid product-detail-metrics"><article class="card"><h3>库存</h3><strong class="metric-${status(product.inventoryLevel)}">${s(product.inventory)}</strong><span class="card-desc">${s(product.inventoryStatus)}</span></article><article class="card"><h3>支付金额</h3><strong>${s(product.paymentAmount)}</strong><span class="card-desc">客单价 ${s(product.avgOrderValue)}</span></article><article class="card"><h3>ROI</h3><strong>${s(product.roi)}</strong><span class="card-desc">广告消耗 ${s(product.adSpend)}</span></article><article class="card"><h3>毛利率</h3><strong>${s(product.grossMargin)}</strong><span class="card-desc">毛利 ${s(product.grossProfitAmount)}</span></article></section>${renderPosition(product)}${renderMetricSections(product)}${renderTrafficFacts(product)}${renderTaskSummary(product)}${alertPanel(product)}<section class="page-section product-detail-section"><div class="section-header"><h3>商品页边界</h3><span class="status-badge">资产定位</span></div><p>商品页只展示商品是谁、在哪里、有哪些指标事实、历史任务摘要。完整的交叉验证、SOP 和提交证明在任务详情页处理。</p></section>`;
  }
  function apiError(error) {
    return `<section class="product-toolbar"><div><p class="eyebrow">PRODUCT ARCHIVE · V12.1.2</p><h2>商品档案</h2></div></section><section class="page-section"><div class="section-header"><h3>接口异常</h3><span class="status-badge">无本地兜底</span></div><p>后端接口没有返回可用数据，页面已停止展示本地模拟业务内容。</p><strong>当前页面接口 ${s(error?.message || error || "请求失败")}</strong></section>`;
  }
