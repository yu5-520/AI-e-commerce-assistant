(function () {
  ...
  function alertPanel(product) { const state = product.alertState || {}; if (!state.activeAlertCount) return `<section class="page-section product-detail-section"><div class="section-header"><h3>商品标签</h3><span class="status-badge">观察</span></div><p>当前未进入执行任务，低风险信号已沉淀为商品标签。</p>${tagList(product)}</section>`; const latest = state.latestAlert || {}; const evidence = (latest.evidence || []).map((item) => `<li>${s(item.label)}：${s(item.value)}</li>`).join(""); return `<section class="page-section product-detail-section"><div class="section-header"><h3>报表触发信号</h3><span class="status-badge danger">${s(latest.priority || "中")}</span></div><p>${s(latest.alertType || "报表信号")} · ${s(latest.dataVersion || "数据版本")}</p><ul>${evidence}</ul></section>`; }
  ...
  function scopeHero(rows = []) { if (!currentScope?.fromStore) return `<section class="product-archive-hero"><div><p class="eyebrow">PRODUCT ARCHIVE · V14.8.2</p><h2>商品档案</h2><p>展示当前账号可见的商品定位、系统编码和指标事实。</p></div><div class="product-scope-panel"><span>商品</span><strong>${rows.length}</strong><small>全局商品档案</small></div></section>`; return `<section class="product-archive-hero scoped"><div><p class="eyebrow">STORE PRODUCT ARCHIVE · V14.8.2</p><h2>${s(currentScope.storeName || "店铺商品档案")}</h2><p>${s(currentScope.platform || "平台")} · 当前店铺 ${rows.length || currentScope.productCount || 0} 个商品 · 店铺级商品档案</p>${smallTags([currentScope.storeWeightTag, ...(currentScope.businessTags || [])])}</div><div class="product-scope-panel"><span>执行任务</span><strong>${s(currentScope.activeTaskCount || 0)}</strong><small>${s((currentScope.productRoleTags || [])[0] || "商品状态")}</small><button type="button" class="secondary" data-clear-filter>全部商品</button></div></section>`; }
  ...
  function renderPosition(product) { const pos = product.productPosition || {}; const rows = [["系统店铺编码", pos.systemStoreCode], ["系统SPU编码", pos.systemSpuCode], ["系统LINK编码", pos.systemLinkCode], ["系统SKU编码", pos.systemSkuCode], ["平台", pos.platform || product.platform], ["店铺", pos.storeName || product.store], ["商品ID", pos.productId || product.productId], ["SKU ID", pos.skuId || product.skuId], ["ERP编码", pos.erpProductCode || product.erpProductCode], ["商品链接", pos.productLink || product.productLink || product.link]].filter(([, value]) => value && value !== "—"); return `<section class="page-section product-detail-section"><div class="section-header"><h3>商品定位</h3><span class="status-badge">系统编码</span></div><div class="product-position-grid">${rows.map(([label, value]) => `<div><span>${s(label)}</span><strong>${s(value)}</strong></div>`).join("")}</div></section>`; }
  function factDate(product, item = {}) { return item.dataDate || item.statDate || product.resolvedReportDate || product.metricFactSummary?.dataDate || ""; }
  function renderMetricSections(product) { const sections = (product.metricSections || []).filter((section) => Array.isArray(section.items) && section.items.length); if (!sections.length) return `<section class="page-section product-detail-section"><div class="section-header"><h3>指标事实</h3><span class="status-badge">待入库</span></div><p>当前商品还没有可展示的独立指标事实，请先完成报表导入。</p></section>`; return `<section class="page-section product-detail-section"><div class="section-header"><h3>指标事实</h3><span class="status-badge">${s(product.metricFactSummary?.factCount || 0)} 条</span></div><div class="product-fact-section-list">${sections.map((section) => `<article class="product-fact-section"><h4>${s(section.title)}</h4><div class="product-fact-grid">${section.items.map((item) => `<div><span>${s(item.metricName)}</span><strong>${s(item.displayValue)}</strong><small>${s(factDate(product, item))}</small></div>`).join("")}</div></article>`).join("")}</div></section>`; }
