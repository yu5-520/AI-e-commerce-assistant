(function () {
  ...
  function row(task, index, focusTaskId = "") {
    ...
    return `<article class="todo-card ${focused ? "focused-task" : ""}" data-task-card="${s(task.id)}">
      ...
      <div class="todo-title-cell">
        <div class="todo-thumb">${s(task.imageLabel || "任")}</div>
        <div class="todo-title-block">
          <strong>${s(task.title || task.productTitle)}</strong>
          <small>${s(task.productId || task.entityId || task.id)} · ${s(task.platform || "经营单元")} · ${s(task.store || task.storeName || "任务池")}</small>
          <span>${s(queue)} · ${s(layerName(task))} · ${s(workflow)}</span>
        </div>
      </div>
      <div class="todo-task-block">
        <span>任务动作</span>
        <strong>${s(task.task || task.taskType || "处理经营任务")}</strong>
        <small>触发依据：${s(taskBasis(task))}</small>
        <small>处理要求：${s(a(task.executionRequirements)[0] || task.roleSummary || "查看详情后按证据提交处理结果。")}</small>
      </div>
      <div class="todo-meta-strip">
        <div class="todo-number-cell ${AppShell.statusClass(task.priorityLevel)}"><span>优先级</span><strong>${s(task.priority)}</strong><small>${s(task.deadline)}</small></div>
        <div class="todo-number-cell"><span>执行人</span><strong>${s(task.assigneeName || "未派发")}</strong><small>${s(task.assignedByName || "系统路径确认")}</small></div>
        <div class="todo-number-cell warning"><span>当前状态</span><strong>${s(workflow)}</strong><small>${s(task.reviewerName || "待复核人")}</small></div>
      </div>
  ...
  function visibleTaskQueue(tasks) {
    return tasks.filter((task) => !["backend_tag", "store_product_tag"].includes(task.queueType) && task.displayState !== "backend_only");
  }
  ...
  window.TodoPage = { route: "business-actions", title: "待办", render(ctx) { const focusTaskId = ctx?.state?.focusTaskId || ""; const allTasks = AppTaskStore.listTasks(); const active = AppTaskStore.listActiveTasks(); const tasks = visibleTaskQueue(active); const counters = AppTaskStore.counters?.() || {}; const user = AppApi.currentUser?.() || {}; return `<section class="todo-toolbar"><div><p class="eyebrow">TASK CENTER · V11 MVP</p><h2>任务处理</h2><p>当前以 ${s(user.roleName || "默认账号")} 查看执行任务。低风险结果已沉淀为商品/店铺标签，不进入任务栏制造压力。</p></div><div class="todo-filter-row"><button type="button" data-reset>重置运行态</button></div></section>${notice ? AppShell.notice("操作结果", notice) : ""}<section class="kpi-grid todo-metrics">${metrics(allTasks, active, counters).map(([x,y,z]) => AppShell.metricCard(x,y,z)).join("")}</section><section class="page-section todo-list-section"><div class="section-header"><h3>执行队列</h3><span class="status-badge">${tasks.length} 个可执行任务</span></div><div class="todo-card-list">${tasks.length ? tasks.map((task, index) => row(task, index, focusTaskId)).join("") : `<div class="todo-empty">当前账号没有需要立即处理的执行任务。低风险信号已进入商品/店铺标签。</div>`}</div></section>`; }, mount(ctx) { focusTask(ctx.state?.focusTaskId); ctx.delegate("[data-task-report]", "click", (_, node) => AppTaskActions.openTaskReport(node.dataset.taskReport)); ctx.delegate("[data-assign]", "click", async (_, node) => { await AppApi.assignTodo(node.dataset.assign, { note: "派发任务给运营账号" }); await refresh("任务已派发。"); }); ctx.delegate("[data-accept]", "click", async (_, node) => { await AppApi.acceptTodo(node.dataset.accept, { note: "运营已接收任务" }); await refresh("任务已接收，进入处理中。"); }); ctx.delegate("[data-submit-evidence]", "click", async (_, node) => { await AppApi.submitEvidenceTodo(node.dataset.submitEvidence, collectEvidence(node.dataset.submitEvidence)); await refresh("执行证据已提交，等待总管复核。"); }); ctx.delegate("[data-review-evidence]", "click", async (_, node) => { const [taskId, decision] = node.dataset.reviewEvidence.split(":"); await AppApi.reviewEvidenceTodo(taskId, collectReview(taskId, decision)); await refresh(decision === "approve" ? "复核通过。" : "已退回补充。"); }); ctx.delegate("[data-complete]", "click", async (_, node) => { await AppApi.completeTodo(node.dataset.complete); await refresh("任务已确认。"); }); ctx.delegate("[data-reset]", "click", async () => { await AppApi.resetRuntimeData?.(); await refresh("运行态已清空。"); }); ctx.addCleanup(AppTaskStore.subscribe(() => AppRouter.schedule("todo-store"))); } };
})();