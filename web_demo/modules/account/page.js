(function () {
  ...
  window.AccountPage = {
    ...
    async render() {
      ...
      return `<section class="report-hero"><div><p class="eyebrow">ACCOUNT · V2.3.3</p><h2>${s(label(view.headline || "账号中心"))}</h2><p>${s(label(view.summary || "账号页只展示当前身份、范围和权限入口；具体经营统筹进入对应功能模块。"))}</p></div><div class="report-hero-side"><span>当前身份</span><strong>${s(current.roleName)}</strong><small>${s(current.name)}</small></div></section><section class="kpi-grid report-metrics">${[["当前角色", current.roleName, current.insightName], ["数据范围", current.scope, "按角色授权"], ["可见模块", (current.visibleModules || []).length, "动态导航"], ["操作权限", actions.length, "按钮级控制"]].map(([a,b,c]) => AppShell.metricCard(a,b,c)).join("")}</section><section class="page-section"><div class="section-header"><h3>我的权限摘要</h3><button type="button" data-role-console>角色权限管理</button></div>${chips(current.permissionNames || [])}<div class="info-list"><div><span>可操作</span><strong>${s(actions.join(" / "))}</strong></div><div><span>不可访问</span><strong>${s((current.hiddenFields || []).join(" / ") || "无")}</strong></div></div></section><section class="page-section"><div class="section-header"><h3>我的功能入口</h3><span class="status-badge">${s(current.insightName)}</span></div><div class="report-card-list">${sections.map((item) => `<article class="report-card"><strong>${s(item)}</strong></article>`).join("")}</div></section>`;
    },
    mount(ctx) { ctx.delegate("[data-role-console]", "click", () => AppRouter.navigate("role-console")); },
  };
