(function () {
  ...
  const label = (value) => String(value || "").replaceAll("任务指挥", "人员总览").replaceAll("利润预算", "供投财务").replaceAll("复核审计", "复盘审计");
  function chips(items = []) { return `<div class="permission-chip-row">${items.map((item) => `<span>${s(label(item))}</span>`).join("")}</div>`; }
  function rolePermissionCard(role, permissions) { const selected = new Set(role.permissions || []); return `<article class="report-card"><div class="section-header"><h3>${s(role.name)}</h3><span class="status-badge">L${s(role.level)}</span></div><div class="permission-chip-row">${permissions.map((permission) => `<button type="button" class="secondary" data-permission="${s(role.id)}:${s(permission.id)}" aria-pressed="${selected.has(permission.id)}">${selected.has(permission.id) ? "✓ " : ""}${s(label(permission.name))}</button>`).join("")}</div></article>`; }
  ...
  window.RoleConsolePage = {
    ...
    async render() {
      ...
      const roles = payload.roles || [], users = payload.users || [], stores = payload.stores || [], permissions = payload.permissions || [], logs = payload.roleChangeLogs || [];
