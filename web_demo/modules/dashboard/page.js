(function () {
  ...
  function localFallback() {
    const tasks = AppTaskStore.listActiveTasks().filter((task) => task.displayState !== "backend_only" && !["backend_tag", "store_product_tag", "observe_candidate"].includes(task.queueType)).map((task, index) => normalizeTask(task, index + 1));
    ...
    return {
      ...
      latestImport: { label: "最新数据", status: "待同步", totalRows: 0, importedCount: 0, affectedModules: [] },
      metrics: [
        { label: "优先任务", value: tasks.length, desc: "今日先处理" },
        { label: "高风险", value: highRisk.length, desc: "执行队列" },
        { label: "店铺", value: 0, desc: "经营对象" },
        { label: "商品", value: 0, desc: "已入库" },
      ],
      taskQueue: tasks,
      todayWorkbench: {
        ...
        latestReportResult: { label: "最新数据", status: "待同步", summary: "上传报表后自动清洗、入库并生成标签。", taskHint: `${tasks.length} 个执行任务`, latestSyncedAt: "待同步" },
  ...
  function normalizeTask(task, rank) {
    ...
    const title = card.title || task.productId || task.entityId || task.productShort || "任务";
    const subtitle = card.subtitle || detail.warningSummary || task.riskDomain || task.taskType || "SOP任务";
    return { rank, id: task.id, title, subtitle, riskDomain: task.riskDomain || subtitle, priority: task.priority || card.priority || "中", priorityLevel: task.priorityLevel || "warning", deadline: task.deadline || card.deadline || "本周内", status: task.workflowStatus || task.status || "待处理", source: task.source || task.sourceModule || "任务池", reason: subtitle, assigneeName: task.assigneeName || "未派发" };
  }
  ...
  function taskRow(task) {
    return `<article class="dashboard-task-card dashboard-compact-task-row">
      <div class="todo-rank ${AppShell.statusClass(task.priorityLevel)}">${s(task.rank)}</div>
      <div class="dashboard-compact-task-main">
        <div class="dashboard-compact-task-title"><strong>${s(task.title)}</strong><span>${s(task.priority)}</span></div>
        <p>${s(task.subtitle || task.reason)}</p>
        <small>${s(task.deadline)} · ${s(task.assigneeName)} · ${s(task.status)}</small>
      </div>
      <div class="dashboard-linked-actions"><button type="button" data-open-task="${s(task.id)}">处理</button></div>
    </article>`;
  }
