(function () {
  ...
  function parseCsv(text) {
    const rows = []; let row = []; let cell = ""; let quoted = false; const source = String(text || "").replace(/^\uFEFF/, "");
    for (let index = 0; index < source.length; index += 1) {
      const char = source[index]; const next = source[index + 1];
      if (char === '"' && quoted && next === '"') { cell += '"'; index += 1; continue; }
      if (char === '"') { quoted = !quoted; continue; }
      if (char === "," && !quoted) { row.push(cell.trim()); cell = ""; continue; }
      if ((char === "\n" || char === "\r") && !quoted) { if (char === "\r" && next === "\n") index += 1; row.push(cell.trim()); if (row.some((item) => item !== "")) rows.push(row); row = []; cell = ""; continue; }
      cell += char;
    }
    row.push(cell.trim()); if (row.some((item) => item !== "")) rows.push(row); if (rows.length < 2) return [];
  ...
  function uploadCard() { const status = previewResult ? (previewResult.status === "ready" ? "字段已识别" : previewResult.status === "needs_attention" ? "字段需确认" : "字段缺失") : "等待上传"; return `<div class="report-upload-card"><div><span>主流程 · ${s(status)}</span><strong>上传报表</strong><small>上传后先检查字段，确认后再生成预警</small></div><label class="report-upload-field"><span>报表类型</span><select data-report-type>${datasetOptions.map(([value, label]) => `<option value="${s(value)}" ${value === selectedDataset ? "selected" : ""}>${s(label)}</option>`).join("")}</select></label><button type="button" data-upload-report>选择文件并预检</button><input id="reportFileInput" type="file" accept=".csv,text/csv" data-report-file hidden /><small class="report-upload-tip">${s(selectedFileName || "支持 CSV。预检通过后再确认导入，避免错表直接生成任务。")}</small><button type="button" class="ghost" data-v3-demo>备用：使用示例数据试跑</button></div>`; }
  function v3Metrics() { const data = v3(); return [["新增预警", data.activeAlertCount || 0, "报表触发"], ["高风险", data.highPriorityAlertCount || 0, "优先处理"], ["已进待办", data.taskLinkedAlertCount || 0, "任务同步"], ["最近版本", shortVersion(data.latestDataVersion), data.latestDatasetName || "等待报表"]].map(([a,b,c]) => AppShell.metricCard(a,b,c)).join(""); }
  ...
  async function confirmImport() {
    ...
    notice = "正在确认导入并生成预警..."; AppRouter.schedule("report-confirm-start");
    const result = await AppApi.confirmReportImport(selectedDataset, pendingRows, previewResult.fieldMapping || {});
    ...
    previewResult = null; pendingRows = [];
  ...
  window.ReportPage = {
    ...
    render() {
      ...
      return `<section class="report-hero"><div><p class="eyebrow">REPORT CENTER · V3.0.2</p><h2>ERP / CRM 报表管理</h2><p>上传订单、退款、库存、商品报表后，系统先识别字段并预览数据，确认导入后再生成预警和待办。</p></div>${uploadCard()}</section>${notice ? AppShell.notice("操作结果", notice) : ""}${previewPanel()}<section class="kpi-grid report-metrics">${v3Metrics()}</section>${(v3().latestAlerts || []).length ? `<section class="page-section report-section"><div class="section-header"><h3>最新预警</h3><span class="status-badge">${s((v3().latestAlerts || []).length)} 条</span></div><div class="report-card-list">${v3().latestAlerts.map((alert) => `<article class="report-card"><div><h3>${s(alert.alertType)}</h3><p>${s(alert.entityId)} · ${s(alert.riskDomain)} · ${s(alert.priority)}</p><div class="report-meta"><span>${s(alert.sourceDataset)}</span><span title="${s(alert.dataVersion)}">${s(shortVersion(alert.dataVersion))}</span><span>${s(alert.status)}</span></div></div><div class="report-actions">${alert.taskId ? `<button type="button" data-open-task="${s(alert.taskId)}">查看待办</button>` : ""}</div></article>`).join("")}</div></section>` : ""}${AppMockData.reportGroups.map((group) => `<section class="page-section report-section"><div class="section-header"><h3>${s(group.title)}</h3><span class="status-badge">可查看</span></div><div class="report-card-list">${group.reports.map(card).join("")}</div></section>`).join("")}`;
    },
    mount(ctx) {
