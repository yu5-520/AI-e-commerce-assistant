(function () {
  ...
  function latestReport(payload) {
    ...
    if (!payload?.hasData && !sync && !records.length) return { label: "等待同步", status: "待同步", taskCount: 0, rows: 0, message: "暂无同步数据", modules: [] };
    return { label: sync?.datasetNames?.join(" / ") || first.name || first.label || payload?.v3?.latestDataVersion || "等待同步", status: sync?.status === "completed" ? "已更新" : first.status || (payload?.v3?.latestDataVersion ? "已更新" : "待同步"), taskCount: sync?.createdTaskCount ?? first.createdTaskCount ?? first.taskCount ?? payload?.recentAlerts?.length ?? 0, rows: sync?.rowCount ?? first.rows ?? first.totalRows ?? 0, message: sync?.userMessage || sync?.summary, modules: sync?.updatedModuleLabels || [] };
  }
  ...
  function stationNode(station = {}, index = 0) {
    return `<div class="metro-station metro-${s(station.status || "waiting")}" style="--i:${index}"><span class="metro-dot"></span><strong>${s(station.label)}</strong><small>${s(station.note || "")}</small></div>`;
  }
  ...
  function emptyRecordRow() { return `<article class="report-record-row"><strong>暂无同步记录</strong><span>等待接口同步</span><span>清空后不保留占位记录</span><button type="button" data-source-sync="erp">同步 ERP</button></article>`; }
  ...
  function sourceCard(item) {
    ...
    const action = isBackup ? `<button type="button" class="secondary" data-open-upload>选择文件</button>` : `<button type="button" data-source-sync="${s(item.sourceId)}">${s(item.actionLabel || "同步数据")}</button><button type="button" class="secondary" data-open-source-config="${s(item.sourceId)}">接口配置</button>`;
    return `<article class="platform-card data-source-card"><div class="platform-head"><div><span class="status-dot ${statusClass}"></span><strong>${s(item.label)}</strong></div><span>${s(isBackup ? "备用" : "主链路")}</span></div><div class="platform-numbers"><div><small>状态</small><b>${s(item.displayStatus || "待配置")}</b></div><div><small>频率</small><b>${s(item.cadence || "按需")}</b></div></div><footer>${s(scope)} → ${s(modules)}</footer><div class="task-actions">${action}</div></article>`;
  }

  function uploadSection() { return `<section class="page-section v102-main-section"><div class="section-header"><h3>备用上传</h3><span class="status-badge">兜底入口</span></div><input type="file" data-manual-file-input accept=".xlsx,.xlsm,.xls,.csv,.json,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,text/csv,application/json" style="display:none" /><div class="report-record-list"><article class="report-record-row"><strong>手动上传 Excel / CSV / JSON</strong><span>备用补数</span><span>接口未开通或异常时使用</span><button type="button" class="secondary" data-open-upload>选择文件</button></article><article class="report-record-row"><strong>演示数据同步</strong><span>Demo</span><span>生成测试任务和记录</span><button type="button" class="secondary" data-import-demo>运行演示</button></article><article class="report-record-row"><strong>清空测试数据</strong><span>Demo</span><span>重置总览、经营、任务、数据和日志</span><button type="button" class="secondary" data-reset-demo>清空数据</button></article></div></section>`; }

  function parseCsv(text) { const rows = []; let row = []; let field = ""; let inQuotes = false; for (let index = 0; index < text.length; index += 1) { const char = text[index]; const next = text[index + 1]; if (char === '"') { if (inQuotes && next === '"') { field += '"'; index += 1; } else inQuotes = !inQuotes; } else if (char === "," && !inQuotes) { row.push(field); field = ""; } else if ((char === "\n" || char === "\r") && !inQuotes) { if (char === "\r" && next === "\n") index += 1; row.push(field); if (row.some((cell) => String(cell).trim() !== "")) rows.push(row); row = []; field = ""; } else field += char; } row.push(field); if (row.some((cell) => String(cell).trim() !== "")) rows.push(row); const headers = (rows.shift() || []).map((header) => String(header || "").trim()); if (!headers.length) return []; return rows.map((values) => { const item = {}; headers.forEach((header, index) => { if (header) item[header] = values[index] ?? ""; }); return item; }).filter((item) => Object.values(item).some((value) => String(value).trim() !== "")); }
  async function parseUploadFile(file) { const text = await file.text(); if (/\.json$/i.test(file.name || "")) { const payload = JSON.parse(text); const rows = Array.isArray(payload) ? payload : payload.rows || payload.data || []; if (!Array.isArray(rows)) throw new Error("JSON 需要是数组，或包含 rows/data 数组。"); return rows.filter((item) => item && typeof item === "object"); } return parseCsv(text); }
  function uploadSummary(result, file) { const meta = result?.uploadMeta || {}; const rows = meta.totalRows ?? result?.rowCount ?? result?.v104ImportTaskSync?.rowCount ?? 0; const sheetText = meta.sheetCount ? `，识别 ${meta.sheetCount} 个 Sheet` : ""; return `备用上传完成：${file.name}，读取 ${rows} 行${sheetText}。`; }
  function setSourceMessage(text) { sourceMessage = text; const node = AppShell.view()?.querySelector("[data-source-message]"); if (node) node.textContent = text; }

  window.ReportPage = {
    route: "data-check",
    title: "数据",
    async render() {
      const [payload, connectionPayload, linePayload] = await Promise.all([AppApi.report(), AppApi.dataSourceConnections?.(), AppApi.dataLineView?.()]);
