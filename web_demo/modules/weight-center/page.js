(function () {
  ...
  async function loadSnapshots(type) { const q = new URLSearchParams({ limit: "120" }); if (type) q.set("object_type", type); return fetchJson(`/api/architecture/v8/weight-snapshots?${q.toString()}`); }
  async function loadComparisons(type, comparisonType) { const q = new URLSearchParams({ limit: "120" }); if (type) q.set("object_type", type); if (comparisonType) q.set("comparison_type", comparisonType); return fetchJson(`/api/architecture/v8/weight-comparisons?${q.toString()}`); }
  async function loadRagHits(type, status) { const q = new URLSearchParams({ limit: "120" }); if (type) q.set("object_type", type); if (status) q.set("hit_status", status); return fetchJson(`/api/architecture/v8/weight-rag-hits?${q.toString()}`); }
  async function loadRelations(type, risk) { const q = new URLSearchParams({ limit: "120" }); if (type) q.set("object_type", type); if (risk) q.set("risk_direction", risk); return fetchJson(`/api/architecture/v8/linked-relations?${q.toString()}`); }
  async function loadScores(type, state) { const q = new URLSearchParams({ limit: "120" }); if (type) q.set("object_type", type); if (state) q.set("weight_state", state); return fetchJson(`/api/architecture/v8/weight-scores?${q.toString()}`); }
  async function loadContext(type, intensity) { const q = new URLSearchParams({ limit: "120" }); if (type) q.set("object_type", type); if (intensity) q.set("task_intensity_level", intensity); return fetchJson(`/api/architecture/v8/context-weights?${q.toString()}`); }
  async function loadCross(type, status) { const q = new URLSearchParams({ limit: "120" }); if (type) q.set("object_type", type); if (status) q.set("validation_status", status); return fetchJson(`/api/architecture/v8/cross-validations?${q.toString()}`); }
  async function loadGroups(type, status) { const q = new URLSearchParams({ limit: "120" }); if (type) q.set("object_type", type); if (status) q.set("group_status", status); return fetchJson(`/api/architecture/v8/weight-task-groups?${q.toString()}`); }
  async function loadApprovals(status, type) { const q = new URLSearchParams({ limit: "120" }); if (status) q.set("approval_status", status); if (type) q.set("object_type", type); return fetchJson(`/api/architecture/v8/weight-approvals?${q.toString()}`); }
  ...
  async function decideApproval(approvalId, decision) {
    const response = await fetch(`/api/architecture/v8/weight-approvals/${encodeURIComponent(approvalId)}/decide`, {
      ...
      body: JSON.stringify({ decision, note: `V8.8 ${decision}` }),
    });
    if (!response.ok) throw new Error(`${response.status} ${response.statusText}`);
    window.alert(decision === "approve" ? "已审批通过，等待 V8.9 执行层" : "已退回/拒绝");
    AppRouter.schedule("weight-approval-decided");
  }

  function metric(label, value, note) { return AppShell.metricCard(label, value, note || "V8.8"); }
  ...
  function groupStatusName(value) { return { pending_approval: "待审批", evidence_review: "证据复核", human_review_draft: "人工复核草案", approved_pending_execution: "已批待执行", rejected: "已拒绝", human_reviewed: "人工已复核" }[value] || value; }
  ...
  function gateName(value) { return { locked: "锁定", ready_for_execution: "待执行层", blocked: "阻断", human_review_only: "仅人工复核" }[value] || value; }
  ...
  function countBlock(title, data = {}, labeler = (v) => v) {
    ...
    return `<section class="page-section report-section"><div class="section-header"><h3>${s(title)}</h3><span class="status-badge">${rows.length}</span></div><div class="version-alert-list">${rows.map(([key, value]) => `<article class="version-alert-row"><strong>${s(labeler(key))}</strong><span>${s(value)}</span><small>V8.8 指标</small></article>`).join("") || `<div class="log-empty">暂无数据。</div>`}</div></section>`;
  }

  function approvalCard(item) {
    const canDecide = ["pending", "evidence_review", "human_review"].includes(item.approvalStatus);
    return `<article class="report-card"><div><h3>${s(item.objectName || item.objectId)} · ${s(item.groupName)}</h3><p>${s(typeName(item.objectType))} · ${s(approvalStatusName(item.approvalStatus))} · 执行门：${s(gateName(item.executionGate))}</p><div class="report-meta"><span class="status-badge">${s(item.priority)}</span><span>审批角色 ${s(item.approvalRole)}</span><span>强度 ${s(item.finalIntensityLevel)}</span><span>${s(item.taskGroupId)}</span></div><p>V8.8 只打开执行门，V8.9 前不得自动执行。</p></div><div class="report-actions">${canDecide ? `<button type="button" data-approval-approve="${s(item.approvalId)}">通过</button><button type="button" data-approval-reject="${s(item.approvalId)}">拒绝</button><button type="button" data-approval-review="${s(item.approvalId)}">退回复核</button>` : `<span class="status-badge">${s(gateName(item.executionGate))}</span>`}</div></article>`;
  }
  function taskGroupCard(item) { const firstTasks = (item.tasks || []).slice(0, 3).map((task) => `<li>${s(task.title)}：${s(task.action)}</li>`).join(""); return `<article class="report-card"><div><h3>${s(item.objectName || item.objectId)} · ${s(item.groupName)}</h3><p>${s(typeName(item.objectType))} · ${s(groupStatusName(item.groupStatus))} · ${s(item.priority)} · 审批：${s(item.approvalRole)}</p><div class="report-meta"><span class="status-badge">${s(item.finalIntensityLevel)}</span><span>任务 ${s(item.taskCount)}</span><span>${s(readinessName(item.readiness))}</span><span>${s(validationName(item.validationStatus))}</span></div><ul>${firstTasks}</ul></div><div class="report-actions"><span class="status-badge">任务组</span></div></article>`; }
  ...
  window.WeightCenterPage = {
    ...
    async render() {
      ...
      return `<section class="report-hero report-hero-clean"><div><p class="eyebrow">WEIGHT CENTER · V8.8</p><h2>权重审批流</h2></div><div class="report-hero-side"><span>当前阶段</span><strong>任务组草案 → 审批 → 执行门</strong><small>仍不自动执行</small></div></section>
      <section class="kpi-grid report-metrics">${metric("审批流", approvals?.approvalCount || 0, "approvals")}${metric("待审批", approvals?.byApprovalStatus?.pending || 0, "pending")}${metric("已通过", approvals?.byApprovalStatus?.approved || 0, "approved")}${metric("执行门", approvals?.byExecutionGate?.ready_for_execution || 0, "gate")}</section>
      <section class="report-preview-grid">${countBlock("审批状态", approvals?.byApprovalStatus || {}, approvalStatusName)}${countBlock("执行门", approvals?.byExecutionGate || {}, gateName)}${countBlock("审批角色", approvals?.byApprovalRole || {})}</section>
      <section class="page-section report-section"><div class="section-header"><div><h3>审批流</h3><p>V8.8 只审批任务组草案并打开执行门，V8.9 前不得自动执行。</p></div><div class="report-actions"><button type="button" data-generate-approval>生成审批流</button><button type="button" data-filter-approval="pending">待审批</button><button type="button" data-filter-approval="approved">已通过</button><button type="button" data-filter-approval="evidence_review">证据复核</button><button type="button" data-filter-approval="">全部</button></div></div><div class="approval-card-list report-card-list">${approvalRows.map(approvalCard).join("") || `<div class="log-empty">暂无审批流。先生成任务组，再生成审批流。</div>`}</div></section>
      <section class="page-section report-section"><div class="section-header"><div><h3>交叉任务组</h3></div><div class="report-actions"><button type="button" data-generate-group>生成任务组</button><button type="button" data-filter-group="pending_approval">待审批</button><button type="button" data-filter-group="evidence_review">证据复核</button><button type="button" data-filter-group="human_review_draft">人工复核</button><button type="button" data-filter-group="">全部</button></div></div><div class="group-card-list report-card-list">${taskGroups.map(taskGroupCard).join("") || `<div class="log-empty">暂无任务组。</div>`}</div></section>
      ...
      <section class="page-section report-section"><div class="section-header"><div><h3>联动关系</h3></div><div class="report-actions"><button type="button" data-generate-relation>生成联动</button></div></div><div class="relation-card-list report-card-list">${relations.slice(0, 12).map(relationCard).join("") || `<div class="log-empty">暂无联动关系。</div>`}</div></section>
      <section class="page-section report-section"><div class="section-header"><div><h3>RAG 标准线命中</h3></div><div class="report-actions"><button type="button" data-generate-rag>生成标准线</button></div></div><div class="rag-card-list report-card-list">${hits.slice(0, 12).map(ragHitCard).join("") || `<div class="log-empty">暂无标准线命中。</div>`}</div></section>
      <section class="page-section report-section"><div class="section-header"><div><h3>周期比较</h3></div><div class="report-actions"><button type="button" data-generate-comparison>生成比较</button></div></div><div class="comparison-card-list report-card-list">${comparisons.slice(0, 12).map(comparisonCard).join("") || `<div class="log-empty">暂无周期比较。</div>`}</div></section>
      <section class="page-section report-section"><div class="section-header"><div><h3>权重快照</h3></div><div class="report-actions"><button type="button" data-generate-weight>生成快照</button></div></div><div class="snapshot-card-list report-card-list">${snapshots.slice(0, 12).map(snapshotCard).join("") || `<div class="log-empty">暂无权重快照。</div>`}</div></section>`;
    },
    mount(ctx) {
      ...
      ctx.delegate("[data-filter-approval]", "click", async (_, node) => { const data = await loadApprovals(node.dataset.filterApproval || ""); document.querySelector(".approval-card-list").innerHTML = (data.approvals || []).map(approvalCard).join("") || `<div class="log-empty">当前筛选暂无审批流。</div>`; });
