(function () {
  ...
  async function generateRagHits() {
    const result = await AppApi.post("/api/architecture/v8/weight-rag-hits/generate", null, {});
    window.alert(`生成标准线命中 ${result.createdCount || 0} 条`);
    AppRouter.schedule("weight-rag-generated");
  }

  function metric(label, value, note) { return AppShell.metricCard(label, value, note || "V8.2"); }
  ...
  function ragHitCard(item) {
    return `<article class="report-card"><div><h3>${s(item.objectName || item.objectId)} · ${s(item.metricLabel || item.metricName)}</h3><p>${s(typeName(item.objectType))} · ${s(hitStatusName(item.hitStatus))} · ${s(item.domain)}</p><div class="report-meta"><span class="status-badge">${s(item.severity)}</span><span>当前 ${s(item.currentValue)}</span><span>标准 ${s(item.operator === "min" ? "≥" : "≤")} ${s(item.standardLine)}</span><span>连续 ${s(item.consecutiveLowCount)}</span></div><p>${s(item.payload?.summary || "V8.2 只判断标准线命中，不生成任务。")}</p></div><div class="report-actions"><span class="status-badge">${s(item.ruleId)}</span></div></article>`;
  }

  function countBlock(title, data = {}, labeler = (v) => v) {
    const rows = Object.entries(data);
    return `<section class="page-section report-section"><div class="section-header"><h3>${s(title)}</h3><span class="status-badge">${rows.length}</span></div><div class="version-alert-list">${rows.map(([key, value]) => `<article class="version-alert-row"><strong>${s(labeler(key))}</strong><span>${s(value)}</span><small>V8.2 指标</small></article>`).join("") || `<div class="log-empty">暂无数据。</div>`}</div></section>`;
  }
  ...
  window.WeightCenterPage = {
    ...
    async render() {
      ...
      const hits = rag?.hits || [];
      return `<section class="report-hero report-hero-clean"><div><p class="eyebrow">WEIGHT CENTER · V8.2</p><h2>权重指标与 RAG 标准线</h2></div><div class="report-hero-side"><span>当前阶段</span><strong>快照 / 周期 / 标准线</strong><small>仍不生成升降权任务</small></div></section>
      <section class="kpi-grid report-metrics">${metric("快照数", data?.snapshotCount || 0, "snapshots")}${metric("比较数", comparison?.comparisonCount || 0, "comparisons")}${metric("标准命中", rag?.hitCount || 0, "rag")}${metric("异常", (rag?.byHitStatus?.below_standard || 0) + (rag?.byHitStatus?.above_risk_line || 0), "risk")}</section>
      <section class="report-preview-grid">${countBlock("标准线状态", rag?.byHitStatus || {}, hitStatusName)}${countBlock("风险等级", rag?.bySeverity || {})}${countBlock("对象类型", rag?.byObjectType || {}, typeName)}</section>
      <section class="page-section report-section"><div class="section-header"><div><h3>RAG 标准线命中</h3><p>V8.2 判断商品、店铺、运营指标是否达标；V8.3 才做联动比对，V8.4 才评分。</p></div><div class="report-actions"><button type="button" data-generate-rag>生成标准线</button><button type="button" data-filter-rag="below_standard">低于标准线</button><button type="button" data-filter-rag="above_risk_line">高于风险线</button><button type="button" data-filter-rag="within_standard">达标</button><button type="button" data-filter-rag="">全部</button></div></div><div class="rag-card-list report-card-list">${hits.map(ragHitCard).join("") || `<div class="log-empty">暂无标准线命中。先生成快照/比较，再生成标准线。</div>`}</div></section>
    ...
    mount(ctx) {
      ...
      ctx.delegate("[data-filter-rag]", "click", async (_, node) => {
        const data = await loadRagHits("", node.dataset.filterRag || "");
        document.querySelector(".rag-card-list").innerHTML = (data.hits || []).map(ragHitCard).join("") || `<div class="log-empty">当前筛选暂无标准线命中。</div>`;
      });
    },
  };
})();