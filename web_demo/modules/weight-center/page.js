(function () {
  ...
  async function loadContext(type, intensity) { const q = new URLSearchParams({ limit: "240" }); if (type) q.set("object_type", type); if (intensity) q.set("task_intensity_level", intensity); return fetchJson(`/api/architecture/v8/context-weights?${q.toString()}`); }
  ...
  function metric(label, value, note) { return AppShell.metricCard(label, value, note || "V8.5"); }
  ...
  window.WeightCenterPage = {
    ...
    async render() {
      ...
      return `<section class="report-hero report-hero-clean"><div><p class="eyebrow">WEIGHT CENTER · V8.5</p><h2>上下文权重修正</h2></div><div class="report-hero-side"><span>当前阶段</span><strong>评分 × 店铺上下文 × 任务强度</strong><small>仍不生成升降权任务</small></div></section>
      <section class="kpi-grid report-metrics">${metric("评分数", score?.scoreCount || 0, "scores")}${metric("修正数", context?.adjustmentCount || 0, "context")}${metric("强动作", (context?.byTaskIntensity?.L4 || 0) + (context?.byTaskIntensity?.L5 || 0), "hard")}${metric("人工复核", (context?.byTaskIntensity?.H1 || 0) + (context?.byTaskIntensity?.H2 || 0) + (context?.byTaskIntensity?.H3 || 0), "human")}</section>
      <section class="report-preview-grid">${countBlock("修正状态", context?.byAdjustedState || {}, stateName)}${countBlock("任务强度", context?.byTaskIntensity || {}, intensityName)}${countBlock("上下文类型", context?.byContextType || {})}</section>
      <section class="page-section report-section"><div class="section-header"><div><h3>上下文修正</h3><p>V8.5 让店铺角色、店铺权重、商品拖累和运营责任影响最终状态与动作强度；V8.7 才生成任务。</p></div><div class="report-actions"><button type="button" data-generate-context>生成修正</button><button type="button" data-filter-context="product">商品</button><button type="button" data-filter-context="store">店铺</button><button type="button" data-filter-context="operator">运营</button><button type="button" data-filter-context="">全部</button></div></div><div class="context-card-list report-card-list">${adjustments.map(contextCard).join("") || `<div class="log-empty">暂无上下文修正。先生成评分，再生成修正。</div>`}</div></section>
      <section class="page-section report-section"><div class="section-header"><div><h3>权重评分</h3><p>V8.4 把联动、标准线和周期比较合成对象权重状态。</p></div><div class="report-actions"><button type="button" data-generate-score>生成评分</button><button type="button" data-filter-score="product">商品</button><button type="button" data-filter-score="store">店铺</button><button type="button" data-filter-score="operator">运营</button><button type="button" data-filter-score="">全部</button></div></div><div class="score-card-list report-card-list">${scores.map(scoreCard).join("") || `<div class="log-empty">暂无评分。</div>`}</div></section>
