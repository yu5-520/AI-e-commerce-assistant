(function () {
  ...
  function actionButtons(report) {
    ...
    return `<button type="button" data-back>返回</button><button type="button" data-source="${s(report.sourceRoute)}">返回来源模块</button><button type="button" class="primary" data-create-task="${s(report.module)}:${s(report.entityId)}">加入任务清单</button>`;
  }
  ...
  function renderReport(report) {
    ...
    return `<section class="report-hero"><div><p class="eyebrow">TASK REPORT · ${s(report.sourceModule)}</p><h2>${s(report.title)}</h2><p>${s(report.warningSummary)}</p></div><div class="report-hero-side"><span>风险等级</span><strong>${s(report.riskLevel)}</strong><small>${s(report.taskStatus || report.reportType)}</small></div></section>${notice ? AppShell.notice("操作结果", notice) : ""}<section class="kpi-grid report-metrics">${[["来源模块", report.sourceModule, report.sourceRoute], ["对象 ID", report.entityId, report.module], ["任务状态", report.taskStatus || "候选预警", task.deadline || "待确认"], ["报告时间", "已生成", String(report.generatedAt || "").slice(0, 16)]].map(([a,b,c]) => AppShell.metricCard(a,b,c)).join("")}</section>${evidenceBlock(report.evidence || [])}<section class="page-section"><div class="section-header"><h3>AI 评估</h3><span class="status-badge">人工确认前</span></div><p>${s(report.aiAssessment)}</p><p>${s(report.agentBoundary)}</p></section>${listBlock("建议动作", report.suggestedActions || [])}${listBlock("运营检查清单", report.operationChecklist || [])}${listBlock("需要补充的数据", report.dataNeeded || [])}${listBlock("人工确认点", report.humanDecision || [])}<section class="page-section"><div class="section-header"><h3>下一步</h3><span class="status-badge">执行建议</span></div><p>${s(report.nextStep)}</p><p>${s(report.archiveRule)}</p><div class="report-actions">${actionButtons(report)}</div></section>`;
  }
