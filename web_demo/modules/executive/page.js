(function () {
  ...
  function orgAlerts(account) {
    ...
    return cards("组织配置观察", "GOVERNANCE", [
      ...
      { title: "权限审计", badge: "需记录", state: "关注", text: "角色升降级、店铺授权、权限模板调整会进入复核审计，用于后续追溯。" },
    ]);
  }
  ...
  window.ReviewAuditPage = { route: "review-audit", title: "复核审计", render() { return `${hero("REVIEW AUDIT", "复核审计", "保留派发、提交、复核、权限变更和归档痕迹，用来回看责任链和经营判断。", "日志", logs().length)}${metrics([["日志记录", logs().length, "可追溯"], ["任务归档", logs().filter(x => String(x.status).includes("完成") || String(x.status).includes("归档")).length, "闭环"], ["权限变更", 0, "组织治理"], ["复核记录", logs().filter(x => String(x.type).includes("复核")).length, "管理审计"]])}`; } };
})();